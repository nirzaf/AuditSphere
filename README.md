...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, period/book/basis-bound trial-balance and bounded resumable GL intake, client-scoped accounting dimensions, chart/taxonomy hierarchy controls, QAR setup defaults, GL service-date lineage, opening/movement completeness, deterministic journal-risk review indicators, exact-journal difference-impact classification, governed difference correction states and evaluation metadata, audit planning, typed bank reconciliation and confirmation workflows, package production, archive lineage, provider safety fences, recovery quarantine, queued-operation cancellation, governed financial-package review decisions, exact-byte reviewed package artifacts, deterministic formula-free XLSX/DOCX exports, safe legacy period/chart backfill with append-only ambiguity quarantine, package-bound release candidates, source-bound accounting-evidence freshness, typed specialist schedules, explicit asset-schedule methodology, the client-safe management package view, the authorized staff review queue, source-bound reconciliation-item and typed ageing validation, durable GL/package processing, context-bound adjustment journals, analytical-review replay lineage and reviewer conclusions, secured analytical-review aggregate summaries, booked-versus-calculated valuation differences linked to scoped proposed adjustments, versioned FX ranges and approved rate-rule enforcement, deterministic advanced-consolidation safety cores, and sealed GL source-bound schedule control totals are implemented and verified locally (**220/220 tests passing on PostgreSQL 18.6 across 81 migrations**). Live provider behavior, compliance approval, signing methodology, PDF conversion, production recovery, approved advanced-method fixtures, and independent review remain explicitly blocked and are never fake-passed.
...
As of the last verification pass at `master@5074abc`: **220/220 tests passed** on PostgreSQL 18.6 with 0 skipped, 81 migrations applied (latest: `20260922071325_QuarantineLegacyAccountingBackfill`), locked restore and the Web Release build completed with zero warnings, the loopback restore rehearsal passed with 81 migrations, and the EF model has no pending changes. The verified slice binds legacy dataset periods and mapping charts only when one exact candidate exists; zero or multiple candidates remain unresolved and produce append-only quarantine evidence. The complete local evidence is recorded in [`docs/execution/current-slice.md`](docs/execution/current-slice.md) and [`docs/execution/status.json`](docs/execution/status.json); approved formula-bearing templates, PDF conversion, live provider behavior, compliance approval, signing methodology, production recovery, advanced-method fixtures, and independent review remain recorded blockers.
...
**Delivered (locally verified — 220/220 tests, 81 migrations):**
...
- **Financial-statement production:** Approved versioned mappings, deterministic adjusted-TB snapshots, opening/closing cash-flow bridge, disclosure validations, exact-byte package-artifact persistence with SHA-256 digest verification, deterministic formula-free/macro-free XLSX and DOCX exports, and artifact-bound review decisions. Approved formula-bearing templates and verified PDF conversion remain pending.
