...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, trial-balance and bounded resumable GL intake, audit planning, package production, archive lineage, provider safety fences, recovery quarantine, governed financial-package review decisions, package-bound release candidates, source-bound accounting-evidence freshness, explicit asset-schedule methodology, the client-safe management package view, and the authorized staff review queue are implemented and verified locally (**191/191 tests passing on PostgreSQL 18.6 across 49 migrations**). Live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain explicitly blocked and are never fake-passed.
...
As of the last verification pass: **191/191 tests passed** on PostgreSQL 18.6 with 0 skipped, 49 migrations applied (latest: `20260920222544_AddAssetScheduleMethodology`), local restore and build completed with zero warnings, the loopback restore rehearsal passed with 49 migrations, and the EF model has no pending changes. A redacted non-production evidence note is retained only to document the evidence boundary; tenant identifiers and credentials are not committed. Runtime identity fixtures, live provider behavior, compliance approval, signing, recovery, and independent-review gates remain **recorded blockers**.
...
**Delivered (locally verified — 191/191 tests, 49 migrations):**
