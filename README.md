...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, period/book/basis-bound trial-balance and bounded resumable GL intake, client-scoped accounting dimensions, chart/taxonomy hierarchy controls, QAR setup defaults, GL service-date lineage, opening/movement completeness, deterministic journal-risk review indicators, exact-journal difference-impact classification, governed difference correction states and evaluation metadata, audit planning, package production, archive lineage, provider safety fences, recovery quarantine, governed financial-package review decisions, exact-byte reviewed package artifacts, package-bound release candidates, source-bound accounting-evidence freshness, typed specialist schedules, explicit asset-schedule methodology, the client-safe management package view, the authorized staff review queue, source-bound reconciliation-item and typed ageing validation, durable GL/package processing, context-bound adjustment journals, analytical-review replay lineage and reviewer conclusions, secured analytical-review aggregate summaries, booked-versus-calculated valuation differences linked to scoped proposed adjustments, and representative accounting workload benchmarks are implemented and verified locally (**208/208 tests passing on PostgreSQL 18.6 across 68 migrations**). Live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain explicitly blocked and are never fake-passed.
...
As of the last verification pass at `master@ae9827e`: **208/208 tests passed** on PostgreSQL 18.6 with 0 skipped, 68 migrations applied (latest: `20260921113403_AddAuditDifferenceEvaluationMetadata`), local restore and build completed with zero warnings, the loopback restore rehearsal passed with 68 migrations, and the EF model has no pending changes. The verified slice includes QAR defaults for client accounting/reporting fields, deterministic journal-risk review indicators, exact-journal difference-impact classification, governed difference correction states with materiality/qualitative metadata and stale-impact rejection, and explicit receivable/payable ageing evidence: as-of date, date basis, bucket rule/bucket, credit treatment and paired settlement links; scoped audit-difference summaries preserve gross and signed/net corrected/unadjusted totals by currency. The complete local evidence is recorded in [`docs/execution/current-slice.md`](docs/execution/current-slice.md) and [`docs/execution/status.json`](docs/execution/status.json); live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain recorded blockers.
...
**Delivered (locally verified — 208/208 tests, 68 migrations):**
