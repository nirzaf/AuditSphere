...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, trial-balance and bounded resumable GL intake, audit planning, package production, archive lineage, provider safety fences, recovery quarantine, governed financial-package review decisions, and the client-safe management package view are implemented and verified locally (**188/188 tests passing on PostgreSQL 18.6 across 45 migrations**). Live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain explicitly blocked and are never fake-passed.
