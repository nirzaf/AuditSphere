...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, trial-balance and bounded resumable GL intake, audit planning, package production, archive lineage, provider safety fences, recovery quarantine, governed financial-package review decisions, the client-safe management package view, and the authorized staff review queue are implemented and verified locally (**188/188 tests passing on PostgreSQL 18.6 across 45 migrations**). Live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain explicitly blocked and are never fake-passed.
...
As of the last verification pass: **188/188 tests passed** on PostgreSQL 18.6 with 0 skipped, forty-five migrations applied (latest: `20260920204413_GeneralLedgerStreamingImport`), local restore and build completed with zero warnings, the loopback restore rehearsal passed with 45 migrations, and the EF model has no pending changes. A redacted non-production evidence note is retained only to document the evidence boundary; tenant identifiers and credentials are not committed. Runtime identity fixtures, live provider behavior, compliance approval, signing, recovery, and independent-review gates remain **recorded blockers**.
...
**Delivered (locally verified — 188/188 tests, 45 migrations):**
...
- **Client accounting and package governance:** Client accounting profiles, periods/books, chart-of-accounts mappings, versioned trial-balance profiles and multi-entity batches, typed GL imports, bounded resumable GL chunks with canonical digests and idempotent finalization, paged completeness bridges, reconciliations, specialist workbenches, entity-package projections, restatement lineage, bounded same-currency consolidation, journal lineage, close checks, immutable management/accounting/partner package-review decisions bound to exact package revision, generation and hash, a client-safe validated-package management acknowledgement flow, and an authorized staff review queue.
