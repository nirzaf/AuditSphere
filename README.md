...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, period/book/basis-bound trial-balance and bounded resumable GL intake, audit planning, package production, archive lineage, provider safety fences, recovery quarantine, governed financial-package review decisions, package-bound release candidates, source-bound accounting-evidence freshness, typed specialist schedules, explicit asset-schedule methodology, the client-safe management package view, the authorized staff review queue, source-bound reconciliation-item validation, durable GL/package processing and representative accounting workload benchmarks are implemented and verified locally (**200/200 tests passing on PostgreSQL 18.6 across 57 migrations**). Live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain explicitly blocked and are never fake-passed.
...
As of the last verification pass at `master@10d0a34`: **200/200 tests passed** on PostgreSQL 18.6 with 0 skipped, 57 migrations applied (latest: `20260921042350_BindTrialBalanceToReportingContext`), local restore and build completed with zero warnings, the loopback restore rehearsal passed with 57 migrations, and the EF model has no pending changes. Trial-balance CSV/XLSX imports persist the selected reporting period, optional reporting book and basis and completeness/reconciliation commands reject mismatched source context. The representative accounting benchmark covers four clients, 2,000 transactions, 8,000 GL lines, two concurrent durable workers and a bounded group calculation; timings are development-capacity observations only. A redacted non-production evidence note is retained only to document the evidence boundary; tenant identifiers and credentials are not committed. Runtime identity fixtures, live provider behavior, compliance approval, signing, recovery, and independent-review gates remain **recorded blockers**.
...
**Delivered (locally verified — 200/200 tests, 57 migrations):**
