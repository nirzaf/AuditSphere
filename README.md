...
> **Status: implementation in progress.** This repository is a specification-driven build. Core domain modules, practice management, period/book/basis-bound trial-balance and bounded resumable GL intake, client-scoped accounting dimensions, chart/taxonomy hierarchy controls, QAR setup defaults, GL service-date lineage, opening/movement completeness, audit planning, package production, archive lineage, provider safety fences, recovery quarantine, governed financial-package review decisions, exact-byte reviewed package artifacts, package-bound release candidates, source-bound accounting-evidence freshness, typed specialist schedules, explicit asset-schedule methodology, the client-safe management package view, the authorized staff review queue, source-bound reconciliation-item validation, durable GL/package processing, context-bound adjustment journals, analytical-review replay lineage and reviewer conclusions, secured analytical-review aggregate summaries, booked-versus-calculated valuation differences linked to scoped proposed adjustments, and representative accounting workload benchmarks are implemented and verified locally (**207/207 tests passing on PostgreSQL 18.6 across 66 migrations**). Live provider behavior, compliance approval, signing methodology, production recovery, and independent review remain explicitly blocked and are never fake-passed.
...
As of the last verification pass at `master@4aee311`: **207/207 tests passed** on PostgreSQL 18.6 with 0 skipped, 66 migrations applied (latest: `20260921092708_PersistFinancialPackageArtifacts`), local restore and build completed with zero warnings, the loopback restore rehearsal passed with 66 migrations, and the EF model has no pending changes. Trial-balance CSV/XLSX imports persist the selected reporting period, optional reporting book and basis; client-scoped dimension definitions validate nonblank GL dimension values; chart and taxonomy hierarchy parents are scoped to their version, posting accounts cannot be chart parents, and package mappings must match the approved taxonomy statement section; setup/reporting currency defaults to QAR; capability profiles enforce matching ENTITY_REPORTING/GROUP_REPORTING/AUDIT_ONLY client/group scopes; source/import and FX-policy currencies remain explicit; direct and streaming GL imports preserve optional service dates through canonical digests and archive lineage; completeness bridges can bind prior-period TBs, calculate opening-plus-movement residuals and disclose incomplete extracts; financial packages and context-bound adjustment journals inherit validated period/book/basis/currency context and preserve it through the deterministic accounting lineage; package validation records separate statement cross-cast, accounting-equation, equity/profit, comparative-consistency and note-to-face results, rendered artifacts include key lineage identifiers, and exact rendered bytes are persisted and bound to each package review decision; ECL now persists an explicit booked amount and calculates its difference against that amount, while ECL and inventory evidence can retain a scoped proposed-journal link; analytical reviews retain QAR-defaulted reporting currency, deterministic input snapshots, replay hashes, movement flags, seasonality explanations and reviewer conclusions, while secured aggregate summaries enforce client/engagement or explicit group scope and omit component identifiers; journal-risk evidence retains sample-selection, management-explanation and corroboration fields; completeness, reconciliation and adjustment commands reject mismatched source context. The representative accounting benchmark covers four clients, 2,000 transactions, 8,000 GL lines, two concurrent durable workers and a bounded group calculation; timings are development-capacity observations only. A redacted non-production evidence note is retained only to document the evidence boundary; tenant identifiers and credentials are not committed. Runtime identity fixtures, live provider behavior, compliance approval, signing, recovery, and independent-review gates remain **recorded blockers**.
...
**Delivered (locally verified — 207/207 tests, 66 migrations):**
