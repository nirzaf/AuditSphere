...
This document records the automation strategy and the current CI contract. As of 2026-09-23, the repository contains dedicated API and Playwright E2E projects and an active CI workflow. Fresh Release discovery reconciled 305 cases: Domain 257, API 6, E2E 42. The latest full E2E project run passed 42/42 with no skips. At the prior 304-case checkpoint, the combined solution run passed API 6/6, Domain 257/257 and E2E 41/41; Domain/API source and tests are unchanged, but the combined 305-case command has not been rerun. The 305-case checkpoint adds a persisted proposal-detail and scope/revocation journey. The historical [test catalog](TEST_CASE_CATALOG.md) still inventories its original 246 Domain cases; the API/E2E projects identify their newer scenarios with `CaseId` traits.
...
`EXPECTED_CASES=305` is the current reconciled discovery count across the three projects, not a permanent cap. Legitimate additions/removals require a reviewed count/catalog update; do not reduce it just to pass CI. The textual parser counts all three solution project sections and deliberately includes duplicate display names. The TRX check rejects any nonexecuted outcome; no `continue-on-error` or `dotnet test || true` hides failure. Infrastructure failures should be classified separately in the future structured summary while still failing the required check.
