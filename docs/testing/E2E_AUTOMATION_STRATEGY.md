...
This document records the automation strategy and the current CI contract. As of 2026-09-23, the repository contains dedicated API and Playwright E2E projects and an active CI workflow. The proposal-authorship checkpoint adds PostgreSQL-backed preparer/checker and prior-schema migration-preservation cases plus author readback while retaining the prior browser route/scope journey. Expected Release discovery is 307 cases: Domain 259, API 6, E2E 42; execution results are recorded in the execution ledger after verification. The historical [test catalog](TEST_CASE_CATALOG.md) still inventories its original 246 Domain cases; newer scenarios are identified by `CaseId` traits.
