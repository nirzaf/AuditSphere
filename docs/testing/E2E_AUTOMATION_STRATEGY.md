...
This document records the automation strategy and the current CI contract. As of 2026-09-23, the working tree contains dedicated API and Playwright E2E projects and an active CI workflow. Fresh Release discovery found **296 cases**: Domain 257, API 6, E2E 33. The historical [test catalog](TEST_CASE_CATALOG.md) still inventories its original 246 Domain cases; the API/E2E projects identify their new scenarios with `CaseId` traits.

The documentation inventory baseline was inspected on 2026-09-22 at `b8e47219179ab5cd707d95dfc6e594da37429d85`, with a later source/discovery check at `053aaec315a179082d0535ad4bda3c2706c5900d`. It found **246 Domain cases / 235 methods**. That historical count is not the current solution total. Current evidence finds 296 cases. The full solution attempt passed API 6/6 and Domain 257/257; one existing E2E reviewer-page case was intermittent (32/33), passed alone, and the complete E2E rerun passed 33/33 with 0 skipped. The previous 246/246 result at `dc9cb0b` remains historical [execution evidence](../execution/status.json).
...
`EXPECTED_CASES=296` is the current reconciled discovery count across the three projects, not a permanent cap. Legitimate additions/removals require a reviewed count/catalog update; do not reduce it just to pass CI. The textual parser counts all three solution project sections and deliberately includes duplicate display names. The TRX check rejects any nonexecuted outcome; no `continue-on-error` or `dotnet test || true` hides failure. Infrastructure failures should be classified separately in the future structured summary while still failing the required check.
