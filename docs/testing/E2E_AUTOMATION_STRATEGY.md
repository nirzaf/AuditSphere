...
This document records the automation strategy and the current CI contract. As of 2026-09-23, the repository contains dedicated API and Playwright E2E projects and an active CI workflow. Fresh Release discovery reconciles 309 cases: Domain 259, API 6, E2E 44. The latest full Release build passed with 0 warnings/errors, and separate project runs passed Domain 259/259, API 6/6, and E2E 44/44 with 0 skips. The E2E set includes `AS-PAR-002-FS-QUEUE-REVOKE-01`, which revokes the synthetic reviewer grant while the review queue stays open and verifies that an explicit refresh clears the prior package projection. A prior solution-wide concurrent run had one intermittent accounting-queue load-error (42/43); the aggregate solution test command was not rerun for this checkpoint. The proposal-authorship checkpoint adds PostgreSQL-backed preparer/checker and prior-schema migration-preservation cases plus author readback while retaining the prior browser route/scope journey. The historical [test catalog](TEST_CASE_CATALOG.md) still inventories its original 246 Domain cases; newer scenarios are identified by `CaseId` traits.
...
The [active workflow](../../.github/workflows/ci.yml) pins PostgreSQL 18.6 and action revisions, restores in locked mode, builds Release, checks EF model drift, migrates its disposable database, is configured to reconcile all 309 discovered/executed tests with zero skips, collects Cobertura/TRX, probes readiness, and uploads diagnostics. This workflow has not yet been observed running on GitHub. The longer example below is a **future hardening blueprint**, not the active workflow; it adds stricter runner-owned scenario manifests and artifact controls.
...
`EXPECTED_CASES=309` is the current reconciled discovery count across the three projects, not a permanent cap. Legitimate additions/removals require a reviewed count/catalog update; do not reduce it just to pass CI. The textual parser counts all three solution project sections and deliberately includes duplicate display names. The TRX check rejects any nonexecuted outcome; no `continue-on-error` or `dotnet test || true` hides failure. Infrastructure failures should be classified separately in the future structured summary while still failing the required check.
