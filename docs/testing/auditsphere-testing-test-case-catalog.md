...
Controlling sources: [system specification §§44–47](../auditsphere-requirements-system-specification-current.md#s44), [accounting requirements](../auditsphere-accounting-module-requirements-current.md), [audit-workflow requirements](../task_breakdown/source/auditsphere-audit-source-workflow-gap-closure-user-stories-historical.md), and [M365 onboarding requirements](../auditsphere-m365-onboarding-user-stories.md).
...
| AS-PAR-002-FS-STALE-DOWNLOAD-01 | UI/E2E | Revoking both package-read grants clears the open package/artifact view; the current-session artifact read is denied and the persisted artifact bytes remain unchanged. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-FS-REFRESH-REVOKED-01 | UI/E2E | Revoking the package reader's active grants while the page is open and refreshing clears the package, statement totals and artifact projection, then shows Access unavailable. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-ACCT-RECORD-REVOKED-01 | UI/E2E | Revoking the accounting queue reader's active grants and refreshing the open journal queue removes the protected journal row and shows Access unavailable. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-MAPPING-REFRESH-REVOKED-01 | UI/E2E | Revoking the mapping reader's active grants and refreshing the open mapping clears the mapping, source allocations and comparison data, then shows Access unavailable. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-JOURNAL-REFRESH-REVOKED-01 | UI/E2E | Revoking the journal reader's active grants and refreshing the open journal clears its header, line details and management evidence, then shows Access unavailable. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-ACCT-WORKSPACE-SIBLING-TASK-01 | UI/E2E | An engagement-only accounting user sees the assigned engagement’s task summary but not a sibling engagement’s assignee, even when both tasks share a client period. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-ACCT-RECORD-TABS-01 | UI/E2E | In-app navigation from accounting mappings to adjustments reloads the authenticated scoped journal queue without a document reload. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-FS-QUEUE-REVOKE-01 | UI/E2E | Refreshing an open package-review queue after its reviewer grant is revoked clears the former package projection and shows the current authorization result without reloading the browser document. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-CLIENT-FS-STALE-ROUTE-01 | UI/E2E | Same-document navigation from an authorized client financial package to an unavailable package ID shows a generic unavailable state and hides the prior hash and statement totals. | Characterized locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); the existing route lifecycle cleared the prior view. |
| AS-PAR-002-CLIENT-FS-REVOKE-01 | UI/E2E | Revoking the client package grant while management review is open clears the package hash, totals and decision control in the existing browser document; no review decision is persisted. | Characterized locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Microsoft identity acceptance remains separate. |
| AS-PAR-002-JOURNAL-STALE-ROUTE-01 | UI/E2E | In-app navigation from an authorized journal to an unavailable journal ID shows the generic unavailable state without retaining prior journal details. | Implemented locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-PBC-STALE-ROUTE-01 | UI/E2E | In-app navigation from an authorized PBC inbox to an unauthorized sibling engagement rechecks scope and clears the previous request thread without a document reload. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-CLIENT-PBC-STALE-ROUTE-01 | UI/E2E | Same-document navigation from the assigned client request to another recipient’s request renders a generic unavailable state, hides both request threads, and reloads the assigned request on return; grant revocation and refresh clear its private draft. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-FIRM-ADMIN-REVOKE-01 | UI/E2E | Revoking an invitation grant blocks its stale clipboard action; subsequently revoking the administrator grant clears grant, identity and invitation details on same-document refresh. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-PORTFOLIO-EXPORT-REVOCATION-01 | UI/E2E | An authorized scoped CSV escapes a formula-shaped client name as text; after grant revocation, the next export command clears the previously rendered client row and produces no further CSV payload in the same browser document. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-AUDIT-PLAN-READ-01 | UI/E2E | Audit-plan reads require a current grant for the exact engagement; sibling navigation and grant revocation clear prior content, and a denied command persists no new risk. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-AUDIT-FIELDWORK-STALE-ROUTE-01 | UI/E2E | Same-document navigation from an assigned audit-fieldwork engagement to an unassigned sibling clears the adopted program; navigating back reloads only the authorized program. | Implemented locally in [AuditAndReleaseJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/AuditAndReleaseJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-COMPLETION-STALE-ROUTE-01 | UI/E2E | Same-document navigation from a completion page with a package to an unassigned engagement clears the prior package projection. | Characterized locally in [AuditAndReleaseJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/AuditAndReleaseJourneyTests.cs); the existing page lifecycle already rendered the unavailable state. |
| AS-PAR-002-ADV-CONSOLIDATION-STALE-ROUTE-01 | UI/E2E | Same-document navigation from an authorized advanced consolidation group scope to an unauthorized group clears the prior group name and scope ID. | Characterized locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live provider/group-administration acceptance remains separate. |
| AS-PAR-002-MAPPING-STALE-ROUTE-01 | UI/E2E | Same-document navigation from an authorized accounting mapping to a real sibling-engagement mapping without a grant clears both mapping identities, source dataset ID and private allocation rationale. | Characterized locally in [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-REV-REVOKE-01 | UI/E2E | A review-point disposition rechecks its current grant; revocation clears protected review text and disposition state, and the review point remains unchanged. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live Entra/session revocation acceptance remains separate. |
| AS-PAR-002-REV-STALE-ROUTE-01 | UI/E2E | Same-document navigation from an authorized review point to an unauthorized ID clears its private comment/status, and returning to the authorized ID reloads only that point. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/session revocation acceptance remains separate. |
| AS-PAR-002-ARCH-STALE-ROUTE-01 | UI/E2E | Same-document navigation from an authorized archive to another client’s archive clears the prior manifest and entries; returning reloads the authorized archive only. | Implemented locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live records/provider acceptance remains separate. |
| AS-PAR-002-INV-01 | UI/E2E | A client-scoped finance user sees its invoice, cannot read another client’s invoice, and same-document navigation clears the prior invoice/balance before restoring the authorized invoice. | Implemented locally in [InvoiceScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/InvoiceScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
| AS-PAR-002-ENG-01 | UI/E2E | Same-document navigation from an authorized engagement detail to an unassigned engagement hides the prior client and hold data, and navigation back reloads the authorized detail. | Characterized locally in [ClientScopeJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/ClientScopeJourneyTests.cs); live identity/revocation acceptance remains separate. |
...
| AS-PAR-009-PROPOSAL-MAKER-CHECKER-01 | Database | Proposal revisions record their preparer; self-review and review of an unattributed legacy draft are denied; a distinct authorized Partner review is recorded. | Implemented locally; see `PracticeCrmTests.ProposalReview_RequiresIndependentReviewer_AndRejectsUnknownLegacyAuthor`. |
| AS-PAR-009-PROPOSAL-MIGRATION-01 | Database | Upgrade from the prior schema preserves the legacy proposal and leaves its unknown preparer unattributed. | Implemented locally; see `PracticeCrmTests.ProposalAuthorshipMigration_PreservesLegacyProposalWithoutInventingAuthor`. |
...
Validation of the historical inventory reconciled all 246 rows and 235 method identities against source attributes, including class/file mappings, theory multiplicities and full arguments, observed traits, custom display names, unique IDs and category totals. Fresh solution discovery on 2026-09-24 reports 327 cases across Domain, API and E2E projects; the CI discovery guard and this catalog are aligned to that observed count. Current accounting revocation cases include `AS-PAR-002-FS-REFRESH-REVOKED-01`, `AS-PAR-002-ACCT-RECORD-REVOKED-01`, `AS-PAR-002-MAPPING-REFRESH-REVOKED-01` and `AS-PAR-002-JOURNAL-REFRESH-REVOKED-01`. The E2E project passed 59/59 before the journal case was added; focused journal refresh passed 1/1. The full 60-case E2E rerun and corrected hosted validation remain pending. EF model drift was not rerun because no EF entities changed. Discovery/local test results do not imply live tenant or production acceptance.

Fresh Release discovery on 2026-09-24 reconciles 260 Domain + 6 API + 55 E2E = 321 cases. `Materiality_ApprovalRequiresIndependentReviewer` passes against PostgreSQL; `AS-PAR-009-AGGREGATE-DIFFERENCES-UI-01` passes in the PostgreSQL-backed browser suite, which passed 55/55 with 0 skips. The full Domain project passed 260/260 with 0 skips. Hosted verification for the updated count and aggregate implementation is pending. No live tenant or production acceptance is implied.
