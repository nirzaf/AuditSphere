...
| AS-PAR-002-CLIENT-FS-REVOKE-01 | UI/E2E | Revoking the client package grant while management review is open clears the package hash, totals and decision control; no review decision is persisted. | Implemented locally in [ClientFinancialPackage.razor](../../src/AuditSphereOps.Web/Components/Pages/ClientFinancialPackage.razor) and [FinancialArtifactJourneyTests.cs](../../tests/AuditSphereOps.E2E.Tests/FinancialArtifactJourneyTests.cs); live Microsoft identity acceptance remains separate. |
...
Fresh Release discovery on 2026-09-24 reconciles 259 Domain + 6 API + 54 E2E = 319 cases. The audit-fieldwork, completion, advanced-consolidation and accounting-mapping same-document scope cases pass; the PostgreSQL-backed E2E project passed 54/54 with 0 skips, including the mapping case. Hosted verification is pending. No live tenant or production acceptance is implied.
