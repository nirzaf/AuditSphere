...
Validation of the historical inventory reconciled all 246 rows and 235 method identities against source attributes, including class/file mappings, theory multiplicities and full arguments, observed traits, custom display names, unique IDs and category totals. Fresh solution discovery on 2026-09-24 reports 325 cases across Domain, API and E2E projects; the CI discovery guard and this catalog are aligned to that observed count. Current accounting revocation cases include `AS-PAR-002-FS-REFRESH-REVOKED-01` and `AS-PAR-002-ACCT-RECORD-REVOKED-01`. The E2E project passed 58/58 with 0 skips and the Release solution build passed with 0 warnings/errors after these additions. Hosted CI on the preceding documentation SHA failed only because the expected discovery count was stale (expected 323, observed 324); the corrected workflow still needs a successful hosted run. EF model drift was not rerun because no EF entities changed. Discovery/local test results do not imply live tenant or production acceptance.
