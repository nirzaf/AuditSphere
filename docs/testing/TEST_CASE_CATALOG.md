...
Validation of the historical inventory reconciled all 246 rows and 235 method identities against source attributes, including class/file mappings, theory multiplicities and full arguments, observed traits, custom display names, unique IDs and category totals. Fresh solution discovery on 2026-09-24 reports 326 cases across Domain, API and E2E projects; the CI discovery guard and this catalog are aligned to that observed count. Current accounting revocation cases include `AS-PAR-002-FS-REFRESH-REVOKED-01`, `AS-PAR-002-ACCT-RECORD-REVOKED-01` and `AS-PAR-002-MAPPING-REFRESH-REVOKED-01`. The E2E project passed 59/59 with 0 skips and the Release solution build passed with 0 warnings/errors after these additions. Hosted validation of the preceding 325-case source checkpoint is still running; corrected 326-case hosted validation remains pending. EF model drift was not rerun because no EF entities changed. Discovery/local test results do not imply live tenant or production acceptance.
