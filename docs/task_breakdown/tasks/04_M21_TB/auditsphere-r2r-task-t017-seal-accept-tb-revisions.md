...
updated_at: ""
---
# T017 — Seal and accept TB revisions with replacement protection
...
Promote only fully validated source content to an immutable, explicitly accepted revision.
...
- [T016 — Implement CSV and genuine XLSX validation with safe previews](auditsphere-r2r-task-t016-csv-xlsx-validation-safe-previews.md)
...
1. Keep Sealed and Accepted separate; enforce exact validation report/input identity and all expected row/chunk counts.
2. Guard source acceptance with current authority, period/context, revision/generation and independent decision where required.
3. Update the accepted-source pointer and downstream invalidation atomically while preserving predecessor data.
4. Expose the source register, exact TB rows and handoff to mappings/GL completeness.
...
- Immutable sealed TB revision and scoped accepted-source selection.
- Replacement history/currentness and source-grid handoff.

**Direct consumers unlocked by this task:**

- [T018 — Build resumable GL intake and journal drill-down](../05_M21_GL_Mapping/auditsphere-r2r-task-t018-resumable-gl-intake-journal-drilldown.md)
...
- Sealed membership rejects insert/update/delete.
- A rejected or racing replacement cannot corrupt the accepted pointer.
- A source acceptance invalidates affected consumers in the same transaction.
...
|---|---|
| VP-035-AC01 | Given valid balanced CSV/XLSX data, when committed, then source rows, normalized totals, file metadata/hash and reporting context are retained as one revision. |
| VP-035-AC03 | A rejected import leaves the previous accepted source untouched; a successful replacement preserves it and stales dependent calculations/approvals. |

**Related original stories:** `VP-035`. [Full preserved wording and production interpretation](../../reference/auditsphere-r2r-reference-original-52-acceptance-criteria.md).

**Related integration journeys:** [R2R-AT-01](../../coverage/auditsphere-r2r-tracker-integration-journeys.md#r2r-at-01), [R2R-AT-06](../../coverage/auditsphere-r2r-tracker-integration-journeys.md#r2r-at-06), [R2R-AT-07](../../coverage/auditsphere-r2r-tracker-integration-journeys.md#r2r-at-07), [R2R-AT-19](../../coverage/auditsphere-r2r-tracker-integration-journeys.md#r2r-at-19).
