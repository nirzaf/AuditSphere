...
updated_at: ""
---
# T005 — Implement asynchronous validation and explicit DTO mapping
...
- [T004 — Introduce MediatR facades without changing transaction behavior](004_Introduce_MediatR_facades_without_changing_transaction_behavior.md)
...
1. Implement or adapt Meta, Context, Ref, MutationReceiptDto, OperationTicketDto, typed decision and paging contracts in section 4.4.
2. Run AbstractValidator<T> through ValidateAsync; validators sharing a context run sequentially.
3. Map domain results and validation failures to safe API problems and Blazor ValidationMessageStore fields without leaking tracked entities.
4. Treat database validation as advisory until the command rechecks it in its authoritative transaction; preserve the existing error catalogue.
...
- Shared DTO/result definitions and mapping/error adapters.
- Validator registration and a tested nested-row field-error path.

**Direct consumers unlocked by this task:**

- [T006 — Establish bUnit and the layered verification harness](006_Establish_bUnit_and_the_layered_verification_harness.md)
...
- Empty IDs, unsupported enums, oversized text and invalid revisions fail predictably.
