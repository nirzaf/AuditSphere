...
| Repository snapshot inspected | `master@4aee311` |
...
At `master@4aee311`, the repository records **207/207 PostgreSQL-backed tests passing with 0 skipped**, **66 applied migrations**, a successful loopback restore rehearsal through `20260921092708_PersistFinancialPackageArtifacts`, a zero-warning Release build and no pending EF model changes. Client-scoped branch, cost-centre, department, project and intercompany-counterparty definitions now reject undefined nonblank GL dimension values, including bounded streaming imports; chart hierarchy parent lookups are restricted to the target chart version, posting accounts cannot be chart parents, and cross-version parents are rejected; draft taxonomy nodes can be extended with parents from the same taxonomy version while cross-version parents and cycles are rejected. Accounting setup and reporting objects default blank currency input to QAR; capability profiles enforce matching ENTITY_REPORTING/GROUP_REPORTING/AUDIT_ONLY client/group scopes; source/import and FX-policy currencies remain explicit; ECL persists an explicit booked amount and calculates its difference against that amount, while ECL and inventory evidence can retain same-engagement proposed-journal lineage; analytical-review approvals require a reviewer conclusion retained beside the exact replay snapshot/hash; secured analytical-review aggregate summaries enforce client/engagement or explicit group scope and omit client/component identifiers; direct and streaming GL imports preserve optional service dates through canonical digests and archive lineage; completeness bridges can bind the approved prior-period TB, calculate per-account opening-plus-movement residuals and disclose incomplete extracts. Trial-balance CSV/XLSX imports now require and persist the selected reporting period, optional reporting book and normalized basis; context-bound financial packages and adjustment journals inherit and persist validated period/book/basis/currency identity, reject cross-period book scope and preserve that context through their exact lineage; approved mapping destinations must match the taxonomy statement section; package validation records separate statement cross-cast, accounting-equation, equity/profit, comparative-consistency and note-to-face outcomes, and rendered artifacts include adjusted-snapshot, mapping-version and adjustment-plan lineage identifiers; exact framework/template-bound artifact bytes and SHA-256 lineage are persisted and required by package-review decisions; analytical reviews default blank currency to QAR only for the selected client reporting period, retain deterministic input snapshots and replay hashes, disclose negative/seasonal movement flags, and journal-risk evidence retains sample-selection, management-explanation and corroboration fields; completeness/TB reconciliation commands reject mismatched stored source context. The accounting slice includes client accounting profiles, controlled entity and group period roll-forward, versioned trial-balance and GL ingestion, bounded resumable GL chunks, source-bound reconciliations that bind GL batches to the selected reporting period and currency, reconciliation-item validation bound to source currency/as-of/disposition rules, durable GL completeness and financial-package calculation/rendering with source revision/mapping-plan/package-revision fencing and idempotent re-enqueue, generation-bound ECL/inventory/specialist/analytical evidence reviews, typed asset/payroll/loan/equity/related-party/tax/going-concern forecast schedules, package projections, restatement lineage, bounded same-currency consolidation, a bounded approved foreign-operation translation profile with pinned rate/policy inputs and explicit DIRECT-rate semantics, maker/checker review and per-line FX lineage, group-membership revision pinning and stale-perimeter blocking across component/translation/match/journal writes, method-owner capability gating, approved component-package review gating, exact component period/taxonomy/mapping lineage, stale-run publication rechecks, journal lineage, package-aware period close, immutable reopen/amendment lineage, typed accounting-evidence links to reviewed audit procedure results with workpaper handoffs, structured accounting/group archive lineage, approved-prior-scope group roll-forward lineage, an accounting dashboard with period handoffs, an immutable package-review service, scoped staff evidence/review UI and queue, a client-safe validated-package management acknowledgement route, completion-screen package-bound release-candidate preparation, completion-gate integration, and a PostgreSQL-backed representative workload benchmark covering four clients, 2,000 transactions, 8,000 GL lines, two concurrent durable workers, a 32-line group calculation, six-decimal/high-magnitude amounts and paged reads. Full FX remeasurement/reserve, NCI, acquisition, ownership-change, nested-group and other complex methods still require approved method-specific fixtures; the broader audit-workflow stories and external acceptance gates remain individually unaccepted in the execution ledger.
...
**Gap to establish:** Versioned mappings, deterministic package calculations, taxonomy-section enforcement and individual package validation results are reusable local foundations, not proof that every primary statement, note and comparative review is complete. [R6]
...
[R1]: https://github.com/nirzaf/AuditSphere/blob/4aee311/AGENTS.md
[R2]: https://github.com/nirzaf/AuditSphere/blob/4aee311/docs/SPECIFICATION.md
[R3]: https://github.com/nirzaf/AuditSphere/blob/4aee311/src/AuditSphereOps.Domain/Audit/Audit.cs
[R4]: https://github.com/nirzaf/AuditSphere/blob/4aee311/docs/SPECIFICATION.md#s20
[R5]: https://github.com/nirzaf/AuditSphere/blob/4aee311/docs/SPECIFICATION.md#s23
[R6]: https://github.com/nirzaf/AuditSphere/blob/4aee311/src/AuditSphereOps.Application/Accounting/FinancialStatementCalculator.cs
