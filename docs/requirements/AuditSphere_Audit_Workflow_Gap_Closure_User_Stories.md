...
| Repository snapshot inspected | `master@0c6025c` |
...
At `master@0c6025c`, the repository records **198/198 PostgreSQL-backed tests passing with 0 skipped**, **56 applied migrations**, a successful loopback restore rehearsal through `20260921020828_AddIntercompanyEliminationLineage`, a zero-warning Release build and no pending EF model changes. The accounting slice includes client accounting profiles, controlled entity and group period roll-forward, versioned trial-balance and GL ingestion, bounded resumable GL chunks, source-bound reconciliations that bind GL batches to the selected reporting period and currency, reconciliation-item validation bound to source currency/as-of/disposition rules, durable GL completeness and financial-package calculation with source revision/mapping-plan fencing and idempotent re-enqueue, generation-bound ECL/inventory/specialist/analytical evidence reviews, typed asset/payroll/loan/equity/related-party/tax/going-concern forecast schedules, package projections, restatement lineage, bounded same-currency consolidation, a bounded approved foreign-operation translation profile with pinned rate/policy inputs and explicit DIRECT-rate semantics, maker/checker review and per-line FX lineage, group-membership revision pinning and stale-perimeter blocking across component/translation/match/journal writes, method-owner capability gating, approved component-package review gating, exact component period/taxonomy/mapping lineage, stale-run publication rechecks, journal lineage, package-aware period close, immutable reopen/amendment lineage, typed accounting-evidence links to reviewed audit procedure results with workpaper handoffs, structured accounting/group archive lineage, approved-prior-scope group roll-forward lineage, an accounting dashboard with period handoffs, an immutable package-review service, scoped staff evidence/review UI and queue, a client-safe validated-package management acknowledgement route, completion-screen package-bound release-candidate preparation, and completion-gate integration. Full FX remeasurement/reserve, NCI, acquisition, ownership-change, nested-group and other complex methods still require approved method-specific fixtures; remaining long-running accounting rendering and workload benchmarks are not yet complete; the broader audit-workflow stories and external acceptance gates remain individually unaccepted in the execution ledger.
