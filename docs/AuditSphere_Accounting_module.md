...
**Pinned review commit:** `9f777e2` (method-acceptance, approved-component, stale-run, exact component-lineage, bounded foreign-operation translation, explicit FX direction, group-revision, stale-scope-write, archive-lineage, group-roll-forward, source-bound reconciliation, typed receivable/payable ageing evidence, reconciliation-item source binding, typed gross/net difference summaries, deterministic journal-risk analysis, exact-journal difference-impact classification, governed difference correction states and evaluation metadata, typed bank reconciliation, queued-operation cancellation, trial-balance, financial-package and adjustment-journal reporting-context binding, client-scoped GL dimensions, chart/taxonomy hierarchy scope, posting-parent rejection, capability scope validation, QAR setup defaults, GL service-date lineage, opening/movement completeness, durable GL completeness, taxonomy statement-section validation, financial-package consistency validations, lineage-aware artifact rendering, exact-byte package-artifact persistence, artifact-bound package-review gates, secured analytical-review aggregate reporting, controlled external component-pack reconciliation/approval, client-chart-bound mapping applicability, review-only mapping suggestions, explicit portfolio grant scoping, exact engagement-grant boundaries, scope-bound portfolio search/export, table-action tooltip coverage, shared accounting workbench navigation, draft-restored context header, session-storage/in-memory draft resilience, legacy package metadata compatibility, scoped accounting record queues, group consolidation workbench tabs, safe package-review selection preview, persisted period-linked workflow tasks and deadlines, global accessibility/form metadata hardening, grant-checked period detail navigation, service-permissibility capability gating, separated currency operation calculators, persisted FX/rounding adjustment lineage, versioned exchange-rate effective ranges, approved translation-rate rule enforcement, advanced consolidation calculation safety cores, acquisition fair-value adjustment handling, and sealed GL source-bound schedule control totals)
...
Local verification at `master@9f777e2` is **218/218 PostgreSQL-backed tests passing with 0 skipped**, **80 migrations applied** through `20260921212202_ResolveScheduleControlSource`, a successful loopback restore rehearsal through the same migration, a zero-warning Release build, and no pending EF model changes. GL-backed bank-ledger schedules now require a sealed, scoped, hash-matching GL import batch with complete source-account coverage; the persisted GL control total is derived from those source lines and caller mismatches fail closed. The current implementation also retains deterministic, criteria-versioned journal-risk review indicators for manual, year-end, high-value, reversal and missing-source-origin journals without making automatic fraud findings; linked differences persist exact-journal account/statement effects, profit/equity totals, disclosure buckets, approved mapping lineage and a `journal-impact.v2` hash; correction records retain optional materiality and qualitative-review metadata, support proposed/agreed/rejected/applied/reported states, and reject stale or unsupported impact evidence before verified-reflected status; bank reconciliation records retain independently approved ledger and statement sources, typed timing items, proposed-correction lineage and reviewer conclusions; queued durable operations support bounded administrator cancellation dispositions before worker claim; explicit receivable/payable ageing evidence retains the reconciliation as-of date, supported date basis, versioned bucket rule, computed bucket, explicit credit treatment and paired subsequent-settlement date/reference; external component packs retain raw/normalized provenance, line reconciliation, independent review, return/resubmission history and approved compatibility bridges; mapping versions bind approved client-chart applicability with records-export lineage; and the Portfolio page now applies the actor's active firm-wide, direct-client or exact-engagement RoleGrant boundary to its counters, client index, search/export and release/package lists, with no-scope guidance. Shared form UX adds tooltip coverage for plain table action links, required/optional field guidance, localStorage autosave/restore with sessionStorage and in-memory fallbacks, and one shared lifecycle flush path; the shared layout provides current-route navigation for implemented accounting workbenches, and the accounting workspace pins a draft-restored selected period context without broadening authorization. Period-linked workflow tasks now persist an optional exact reporting-period link and deadline, and the dashboard/period detail show the task owner and status before falling back to PBC data. Capability profiles now require an explicit service route and accepted service-permissibility decision for entity-reporting and audit-only services, while external component packs remain usable for audit-only work without client bookkeeping migration. Currency remeasurement, foreign-operation translation and display-only conversion are distinct calculations; foreign-operation results persist separate foreign-exchange and rounding adjustments and approval recomputes them from the exact source and approved rate. Exchange-rate sets retain source, immutable version, approval and effective range metadata; individual observations retain direction, date and rate type, are rejected outside the declared range, and are accepted only when their rate type is declared by the approved translation policy. QAR remains the default only for blank client accounting/reporting inputs; source/import and FX-policy currencies remain explicit. The advanced acquisition, NCI, ownership-change, nested-group and asset-transfer calculation cores are deterministic, including acquisition fair-value adjustments, but their production profiles remain disabled until approved method-specific fixtures exist. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review; the detailed capability inventory remains in `docs/execution/current-slice.md` and `docs/execution/status.json`.
...
**C6 — schedule reconciliation must resolve trusted reference data.** GL-backed bank-ledger schedules now require a sealed, scoped GL import batch whose raw or normalized hash matches the schedule, whose selected account codes have complete source-line coverage, and whose persisted GL control total is derived from those source lines; a caller-supplied mismatch fails closed. Non-GL statement schedules remain separately evidenced and are not silently treated as GL data. [R9]
