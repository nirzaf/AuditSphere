...
**Pinned review commit:** `d869652` (source-bound valuation review freshness)
...
The local implementation now covers client accounting profiles, periods/books, chart-of-accounts mappings, versioned trial-balance profiles and multi-entity batches, typed GL imports, paged account-completeness bridges, source-bound reconciliations, versioned ECL/inventory evidence, specialist workbenches, entity-package projections, restatement lineage, bounded same-currency consolidation, journal lineage, close checks, immutable financial-package review decisions, package-bound release candidates, and completion-screen candidate preparation. Package-review decisions are stage-bound, exact package revision/generation/hash-bound, and append-only; release candidates now preserve the `FINANCIAL_PACKAGE` target and re-check the current management, accounting and partner decisions before checkpointed issuance. ECL and inventory reviews preserve the reconciliation source hash and client input generation and fail closed when that lineage changes. A restricted client portal now exposes validated package metadata and statement totals and permits signed-in management acknowledgement without exposing internal review history or workpapers; an authorized staff queue lists pending stages and routes to the exact package surface.

Local verification at `master@d869652` is **190/190 PostgreSQL-backed tests passing with 0 skipped**, **47 migrations applied**, a successful loopback restore rehearsal, and a zero-warning build. The current GL intake includes bounded resumable chunks, canonical digests, transactional locking, idempotent retries and contiguous finalization; completion and release now read the same exact-package review decisions that gate the accounting workflow, including the partner-only candidate-preparation action. ECL and inventory review tests reject changed client generations after calculation. The remaining items in this document are implementation backlog or verification work unless the execution ledger explicitly marks them locally verified. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review.
...
| Periods and books | `ClientReportingPeriod`, `ClientReportingBook`, `OpeningBalanceBridge` | Entity + period + basis; prior-period linkage; approved close/reopen and amendments. |
...
- [ ] Close prevents unauthorized changes; reopen requires a recorded decision and creates a new working revision.
...
**Acceptance criteria**
- [ ] Close checks outstanding reconciliations, differences, approvals and required reporting artifacts.
- [ ] A post-close change opens an authorized amendment version; the issued package is preserved.
- [ ] Rollforward copies eligible master/mapping/template structure as a draft and links opening balances to the approved prior period.
...
- **R1:** GitHub `master` metadata and Domain/Application inventory at `d869652`.
...
https://github.com/nirzaf/AuditSphere/tree/d869652
