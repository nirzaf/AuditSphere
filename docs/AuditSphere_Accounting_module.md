...
**Pinned review commit:** `8d7a73d` (current accounting implementation including controlled XLSX formulas, deterministic DOCX/PDF exports, persisted and method-validated advanced-method schedules, consolidation schedule-readiness UI, the candidate advanced-method statement fixture and recorded IFRS method scope; detailed capability evidence remains in `docs/execution/current-slice.md`)
...
Local verification at `master@8d7a73d` is **227/227 PostgreSQL-backed tests passing with 0 skipped** at `dfc7563` (the current change is UI-only), **84 migrations applied** through `20260922104120_AdvancedConsolidationMethodSchedules`, a successful loopback restore rehearsal through the same migration, a zero-warning Release build at `8d7a73d`, and no pending EF model changes. QAR remains the default only for blank client accounting/reporting inputs; source/import and FX-policy currencies remain explicit. Deterministic financial-package exports include versioned XLSX, DOCX and PDF profiles with exact-byte persistence, authorized downloads and a runtime-enforced trusted workbook formula allowlist. Approved advanced-method schedules now persist canonical source/input digests, validate method-specific inputs at approval, support idempotent retry, maker/checker approval and group-revision fencing; the consolidation workbench exposes their scope-bound readiness state. A fixed candidate fixture produces balanced QAR current and comparative advanced-consolidation statements, and IFRS method-owner scope is recorded in `STE-METH-APP-001-addendum`; advanced profiles remain disabled until guarded service integration and complete statement/comparative verification are complete. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review; the detailed capability inventory remains in `docs/execution/current-slice.md` and `docs/execution/status.json`.
...
**Local implementation note (2026-09-22):** a fixed candidate fixture combines the implemented FX translation reserve, acquisition goodwill, NCI rollforward, nested-scope uniqueness and asset-transfer elimination into balanced QAR current and comparative statements. Approved source-bound advanced-method schedules now persist canonical source/input digests, validate method-specific inputs at approval, enforce group-revision fencing, support idempotent retry and maker/checker approval. Method-owner approval `STE-METH-APP-001-addendum` covers the IFRS advanced-method families; guarded service integration and complete statement/comparative verification remain required before any advanced profile is enabled.
