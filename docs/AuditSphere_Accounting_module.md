...
**Pinned review commit:** `86de635` (method-acceptance, approved-component, stale-run, exact component-lineage, bounded foreign-operation translation and group-revision gates)
...
The current system already has significant reusable accounting and audit infrastructure. The remaining gap is not “add an accounting module from zero.” It is **turn the bounded, single-entity TB-to-package implementation into a governed, multi-client accounting and consolidation product**.
...
The local implementation now covers client accounting profiles, periods/books, controlled period roll-forward, chart-of-accounts mappings, versioned trial-balance profiles and multi-entity batches, typed GL imports, paged account-completeness bridges, source-bound reconciliations, generation-bound ECL/inventory/specialist/analytical evidence, typed payroll/loan/equity/related-party/tax/going-concern forecast schedules, explicit asset-schedule depreciation method/useful-life and closing-balance controls, entity-package projections, restatement lineage, bounded same-currency consolidation, journal lineage, close checks, immutable financial-package review decisions, package-bound release candidates, completion-screen candidate preparation, typed accounting-evidence links to reviewed audit procedure results, a scoped evidence queue with exact workpaper handoffs, accounting archive lineage, and an accounting dashboard with period status and roll-forward/restatement navigation. Period close is row-locked and package-aware: matching packages must have current management, accounting and partner approval; authorized reopen creates an immutable append-only amendment record and a new working revision. Controlled roll-forward creates a new draft period, copies eligible reporting books as drafts, and creates an explicit hash/evidence-bound opening bridge without copying prior approvals. Package-review decisions are stage-bound, exact package revision/generation/hash-bound, and append-only; release candidates now preserve the `FINANCIAL_PACKAGE` target and re-check the current management, accounting and partner decisions before checkpointed issuance. Accounting evidence captures the relevant client input generation and fails closed when that generation changes; source-bound ECL and inventory evidence also preserves the reconciliation source hash. Consolidation perimeter approval now requires an independently accepted group capability profile for the selected method, component approval requires current management, accounting and partner package reviews, and each component submission is bound to the package's period basis, taxonomy version and mapping-version lineage; run publication recomputes current component-package/journal inputs so stale runs must be rebuilt. Group membership changes now advance a durable group revision; scopes pin that revision and reject approval/calculation after a perimeter change without rewriting historical memberships or runs. The bounded foreign-operation profile additionally pins an approved rate set and translation policy to the scope, requires maker/checker approval of each foreign component translation, preserves per-line source and FX lineage in the deterministic manifest, and blocks missing or stale rates/packages. A restricted client portal now exposes validated package metadata and statement totals and permits signed-in management acknowledgement without exposing internal review history or workpapers; authorized staff queues expose only the actor's explicit client/engagement scope.

Local verification at `master@86de635` is **198/198 PostgreSQL-backed tests passing with 0 skipped**, **54 migrations applied**, a successful loopback restore rehearsal through `20260921004233_PinGroupRevisionToConsolidationScope`, a zero-warning build, and no pending EF model changes. The current GL intake includes bounded resumable chunks, canonical digests, transactional locking, idempotent retries and contiguous finalization; completion and release now read the same exact-package review decisions that gate the accounting workflow, including the partner-only candidate-preparation action. ECL, inventory, specialist and analytical review tests reject changed client generations after evidence preparation; asset schedules require an explicit depreciation method and positive useful life and persist the calculated closing balance; payroll, loans, equity, related-party, tax and going-concern forecast schedules retain typed inputs and require the relevant evidence before approval. Consolidation tests reject missing or mismatched component period, taxonomy and mapping lineage before approval, include those values in the deterministic run manifest, exercise the bounded approved foreign-operation translation path with maker/checker and stale-input guards, and reject runs when group membership revision changes. Full FX remeasurement/reserve profiles, NCI, acquisition, ownership-change, nested-group and other complex methods remain disabled until method-specific fixtures and approvals exist. The remaining items in this document are implementation backlog or verification work unless the execution ledger explicitly marks them locally verified. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review.
...
| G11 | Corrected / uncorrected differences | `AuditDifference` and reviewer evaluation exist; correction is represented by Boolean and reference text. [R8, R10] | Link exact journal/revision and resulting TB snapshot; do not equate a reference string with proof of reflection. | P1 |
...
| G14 | Reporting periods / comparative / multiple books | Engagement and package period fields exist; firm periods belong to firm ledger. [R3, R5] | Client periods, source basis/book, opening bridge, comparative snapshot, close/reopen and restatement history. | P1 |
...
**C6 — schedule reconciliation must resolve trusted reference data.** `CreateScheduleRequest` supplies `GlControlTotal`, `SourceHash` and `SourceReceiptReference`; the service calculates the residual from that supplied total. Retain this as a controlled input seam only until it is connected to a resolved TB/GL snapshot and verified receipt. Otherwise users can make totals agree without proving which source was reconciled. [R9]

**C7 — correction claims need typed lineage.** The reviewed difference command can mark a difference corrected when it has a nonempty correction-reference string. Link to a permitted journal revision, management response, source posting evidence and recalculated package; show “reported corrected” separately from “verified reflected.” [R10]
...
All story criteria below are proposed and unchecked. Each story inherits the common completion contract: scoped authorization, valid and invalid paths, immutable evidence, explicit revisions, tests on actual PostgreSQL when persistence changes, user-visible outcomes, and no unapproved external effect.
...
**Acceptance criteria**
- [ ] Every import/package identifies client, period, basis/book and currency.
...
**Acceptance criteria**
- [ ] Support signed net balance and separate debit/credit layouts through explicit profiles; retain original values and locale decisions.
- [ ] Preserve original file bytes/hash separately from normalized dataset digest and parser/profile version.
- [ ] Validate headers, date/basis/entity/currency, precision, duplicate rows and balancing totals before promotion.
- [ ] A multi-entity file becomes a controlled batch of independently scoped datasets; no cross-entity balancing or account-key collisions.
- [ ] XLSX parsing rejects executable/macros/external-link evaluation and uses bounded sheet, row, cell and decompression limits.
- [ ] Duplicate detection explains whether bytes or normalized content match; an invalid entity/file does not silently promote other invalid data.
...
**Acceptance criteria**
- [ ] Preserve stable journal/line identity, account, posting/document/service dates, debit/credit, original and functional currency values, party, user/source and reversal references when provided.
- [ ] Map branch, cost centre, department, project and intercompany counterparty using client-specific dimension definitions.
- [ ] Reconcile opening plus movement to closing TB by account, entity, period and basis; disclose incomplete extracts.
- [ ] Flag missing/malformed journal groups, duplicates and out-of-period data rather than inventing counterpart lines.
- [ ] Search/drill-down remains paged, scoped and linked to the raw receipt.
- [ ] Fraud/manual/year-end flags are explainable indicators for review, not automatic fraud findings.
...
**Acceptance criteria**
- [ ] Classify each entry as proposed client-book correction, reporting-only adjustment, presentation reclassification or group-only elimination.
- [ ] Store balanced lines, reason, evidence, applicable period/book, origin and exact revision.
- [ ] Management accept/reject/partial-agreement is a signed-in, version-bound decision or explicitly identified offline evidence—not a reviewer-entered string impersonating management.
- [ ] An independent authorized reviewer approves application; posted/applied history is immutable.
- [ ] Reversals and amendments reference prior entries; recurring templates create new drafts, not automatic approval.
- [ ] Exporting an agreed correction does not mark the client's external ledger posted or reflected without evidence.
...
**Acceptance criteria**
- [ ] Add typed links between difference, journal revision, source-reflection evidence and final adjusted snapshot.
- [ ] Distinguish proposed, agreed, rejected, applied in reporting, reported posted externally and verified reflected.
- [ ] Calculate account, profit, equity and relevant disclosure effects from the exact journal rather than a free-form amount alone.
- [ ] Present gross and signed/net summaries so offsetting differences do not disappear.
- [ ] Retain classification, materiality reference, qualitative concerns and professional evaluation.
- [ ] An unsupported or stale correction reference prevents “verified corrected” status and final checklist completion.
...
**Acceptance criteria**
- [ ] Resolve target GL/TB totals from an exact authorized dataset and account selection, not a user-entered total alone.
- [ ] Record signed reconciling items with reason, aging, evidence, disposition and reviewer; do not hide discrepancies in a plug.
- [ ] Receivable/payable aging has an explicit as-of date, date basis, bucket rules, credit treatment and subsequent settlement links.
- [ ] Bank work distinguishes ledger, statement, timing items and proposed correcting entries.
- [ ] Confirmation responses and alternative procedures preserve respondent/authenticity evidence independently of the client schedule.
- [ ] A changed source marks the reconciliation stale and prevents reuse as current audit evidence.
...
**Acceptance criteria**
- [ ] ECL captures eligible exposure, segmentation, rates/assumptions, overlays, management amounts and reviewed methodology version.
- [ ] Inventory separates quantity/count reconciliation from cost and NRV/obsolescence valuation.
- [ ] Calculated differences reconcile to the booked amount and link to proposed adjustments.
- [ ] Input changes create a new calculation version and stale the previous review.
- [ ] Missing assumptions or unsupported methods block a calculated conclusion; no universal default percentages.
...
**Acceptance criteria**
- [ ] Assets reconcile opening/additions/disposals/depreciation/impairment/closing; methods and useful lives remain explicit.
- [ ] Payroll sample recalculations retain contract assumptions, deductions and bank-payment links; the module does not run payroll.
- [ ] Loans preserve repayments, interest/accrual assumptions, maturity split and covenant inputs.
- [ ] Equity reconciles opening, profit/OCI, capital movements, dividends and closing balances; related-party schedules link to disclosures.
- [ ] Tax schedules use approved jurisdiction/period rules and retained return/payment/correspondence evidence; never assume all clients share one tax rate.
- [ ] Going-concern forecasts retain management ownership, cash/debt inputs, assumptions and sensitivity results; the auditor records the conclusion.
...
**Acceptance criteria**
- [ ] Compare current/prior year, monthly periods and client budgets separately from the firm's engagement budget.
- [ ] Ratio definitions identify inputs, currency, period and denominator basis; divide-by-zero and insufficient data remain visible.
- [ ] Track journal flags, selected samples, management explanations, corroboration and human disposition.
- [ ] Negative/credit balances and seasonal movements are handled explicitly.
- [ ] A conclusion links to exact query parameters and input snapshots for replay.
- [ ] Aggregate reports do not reveal unauthorized client or group component details.
...
**Acceptance criteria**
- [ ] Generate the approved statement of financial position, profit/loss and OCI as applicable, cash flows, changes in equity, comparatives and notes.
- [ ] Every figure drills back to mapping, adjusted balance, source, journal and supporting schedule.
- [ ] Cash flows require actual movement/supplementary inputs; a closing TB alone must not fabricate them.
- [ ] Cross-casts, accounting equation, note-to-face totals, equity/profit and comparative checks produce individual validation results.
- [ ] Separate arithmetic validation, preparation, management approval, audit review and release states.
- [ ] Template/framework effective versions are pinned; export is a reviewed artifact, not simply the existing canonical text dump.
...
**Acceptance criteria**
- [ ] Persistent header shows firm, optional group, legal entity, engagement, period, book, currency and package version.
...
- [ ] Group rollforward carries approved opening consolidation reserves, historical FX and recurring elimination lineage without duplicate application.
- [ ] Archive exports include new accounting/group dependencies; legal hold/disposal behavior follows the existing approved records policy.
...
- [ ] Use the existing durable operation infrastructure for long imports, calculations and rendering; do not add a separate job engine.
- [ ] Cancelled/failed work never publishes a partial accepted dataset or group package.
- [ ] Retry is idempotent; timeout-after-effect preserves uncertainty and reconciliation semantics.
- [ ] Benchmark representative client counts, GL volume, group size, concurrency, precision extremes and connection/lock pressure.
...
- **R1:** GitHub `master` metadata and Domain/Application inventory at `86de635`.
...
- **R15:** `src/AuditSphereOps.Application/Accounting/CurrencyTranslationService.cs`, `ConsolidationService.cs` and `ConsolidationCalculator.cs`: pinned approved foreign-operation translation profile, deterministic per-line FX lineage and group-revision invalidation.
...
https://github.com/nirzaf/AuditSphere/tree/86de635
