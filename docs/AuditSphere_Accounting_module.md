...
**Pinned review commit:** `70f4a59` (current accounting implementation including deterministic XLSX, DOCX and PDF financial-package exports; detailed capability evidence remains in `docs/execution/current-slice.md`)
...
Local verification at `master@70f4a59` is **224/224 PostgreSQL-backed tests passing with 0 skipped**, **83 migrations applied** through `20260922083103_EnableLiveMailProviderAuthority`, a successful loopback restore rehearsal through the same migration, a zero-warning Release build, and no pending EF model changes. QAR remains the default only for blank client accounting/reporting inputs; source/import and FX-policy currencies remain explicit. Deterministic financial-package exports now include versioned XLSX, DOCX and PDF profiles with exact-byte persistence and authorized downloads. The advanced acquisition, NCI, ownership-change, nested-group and asset-transfer calculation cores remain disabled until approved method-specific fixtures exist. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review; the detailed capability inventory remains in `docs/execution/current-slice.md` and `docs/execution/status.json`.
...
**Local implementation note (2026-09-22):** the built-in `financial-package-xlsx.v1`, `financial-package-docx.v1` and `financial-package-pdf.v1` profiles produce deterministic inactive exports from the exact canonical package artifact. PDF output uses PDFsharp/MigraDoc with an embedded OFL font and canonicalized generated identifiers. Each profile persists separate SHA-256-bound bytes for the package revision/generation/hash and exposes authorized Blazor downloads. Approved formula-bearing templates remain pending; tenant, signing and records acceptance remains external.
