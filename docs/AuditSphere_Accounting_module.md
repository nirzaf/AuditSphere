...
**Pinned review commit:** `d82e65b` (method-acceptance, approved-component, stale-run, exact component-lineage, bounded foreign-operation translation, explicit FX direction, group-revision, stale-scope-write, archive-lineage, group-roll-forward, source-bound reconciliation, reconciliation-item source binding and durable GL completeness gates)
...
Local verification at `master@d82e65b` is **198/198 PostgreSQL-backed tests passing with 0 skipped**, **56 migrations applied**, a successful loopback restore rehearsal through `20260921020828_AddIntercompanyEliminationLineage`, a zero-warning Release build, and no pending EF model changes. The current GL intake includes bounded resumable chunks, canonical digests, transactional locking, idempotent retries and contiguous finalization; source-bound reconciliations reject sealed batches whose reporting period or currency differs from the selected period; reconciliation items must match the exact source currency, cannot be future-dated beyond the as-of date, require a disposition and retain computed ageing; GL completeness calculation is also enqueued through the existing durable local operation infrastructure with source revision fencing and repeat-enqueue idempotency; completion and release now read the same exact-package review decisions that gate the accounting workflow, including the partner-only candidate-preparation action. ECL, inventory, specialist and analytical review tests reject changed client generations after evidence preparation; asset schedules require an explicit depreciation method and positive useful life and persist the calculated closing balance; payroll, loans, equity, related-party, tax and going-concern forecast schedules retain typed inputs and require the relevant evidence before approval. Consolidation tests reject missing or mismatched component period, taxonomy and mapping lineage before approval, include those values in the deterministic run manifest, exercise the bounded approved foreign-operation translation path with explicit DIRECT-rate semantics, maker/checker and stale-input guards, reject component/translation/match/journal writes after a group-membership revision change, reject stale runs, and verify approved-prior-scope group roll-forward lineage. The archive regression covers structured export of group membership and the related consolidation/rate/translation lineage collections. Full FX remeasurement/reserve profiles, NCI, acquisition, ownership-change, nested-group and other complex methods remain disabled until method-specific fixtures and approvals exist. Remaining long-running accounting calculations/rendering and workload benchmarks are not yet complete. The remaining items in this document are implementation backlog or verification work unless the execution ledger explicitly marks them locally verified. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review.
...
- [x] Use the existing durable operation infrastructure for the GL completeness calculation, with sealed-source revision fencing and idempotent retries; remaining long imports, calculations and rendering must reuse the same infrastructure rather than add a separate job engine.
...
- **R1:** GitHub `master` metadata and Domain/Application inventory at `d82e65b`.
...
https://github.com/nirzaf/AuditSphere/tree/d82e65b
