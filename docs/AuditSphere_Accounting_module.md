...
The accounting workspace now reads persisted in-scope PBC requests for dashboard owner and due-date handoffs, links periods without package or mapping records to the existing PBC request, and keeps `Not recorded` when no persisted task date exists.
...
- [x] Dashboard shows required/complete/stale/blocked counts, named next owner, persisted due dates when a PBC task exists, an explicit `Not recorded` fallback otherwise, and exact grant-checked period/package/mapping/PBC navigation.
