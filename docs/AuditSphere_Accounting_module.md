...
**Pinned review commit:** `0f9f10c` (current accounting implementation including controlled XLSX formulas, deterministic DOCX/PDF exports, persisted and method-validated advanced-method schedules, guarded advanced current/comparative execution evidence with stale component-input revalidation, consolidation readiness UI, the candidate advanced-method statement fixture and recorded IFRS method scope; detailed capability evidence remains in `docs/execution/current-slice.md`)
...
Local verification at `master@0f9f10c` is **237/237 PostgreSQL-backed tests passing with 0 skipped**, **89 migrations applied** through `20260922125309_AdvancedConsolidationExecution`, a successful loopback restore rehearsal through the same migration, a zero-warning Web Release build, and no pending EF model changes. QAR remains the default only for blank client accounting/reporting inputs; source/import and FX-policy currencies remain explicit. Deterministic financial-package exports include versioned XLSX, DOCX and PDF profiles with exact-byte persistence, authorized downloads and a runtime-enforced trusted workbook formula allowlist. Approved advanced-method schedules persist canonical source/input digests, validate method-specific inputs at approval, and now feed a guarded execution record with balanced current/comparative statement JSON, method output manifest/digest, current component/group-revision checks, idempotent retry and separate reviewer approval that revalidates component inputs. A fixed candidate fixture produces balanced QAR current and comparative advanced-consolidation statements, and IFRS method-owner scope is recorded in `STE-METH-APP-001-addendum`; method-specific golden fixtures and browser journeys remain required before an advanced profile is treated as fully enabled. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review; the detailed capability inventory remains in `docs/execution/current-slice.md` and `docs/execution/status.json`.
...
- [x] Run browser journeys for an SME, unrelated clients with colliding codes, a basic group and each advanced profile's fail-closed gate; current evidence records the four unproven profiles as `REQUIRED` with no executions and the accepted Acquisition/NCI fixture as `APPROVED` with `1 / 1` execution.
...
- [ ] Repeat the full browser journey for each additional advanced profile before enabling it.
...
**Local browser note (2026-09-22):** the explicit Development/Test-only browser harness migrates and seeds the isolated `auditsphere_browser` database with one QAR SME, two unrelated clients that each retain account code `1000` in separate chart scopes, a basic QAR group, an enabled USD-to-QAR group with approved rate-set/policy fixtures and an advanced-method gating group containing all five advanced profiles. Built-in-browser journeys verified client context switching, group/FX status and scope-version rendering. The `ACQUISITION_NCI_V1` fixture completed required-field guidance and local draft restore, preparer schedule submission, independent schedule approval, balanced verified execution and independent execution approval; the main workbench reports `1 / 1` execution and `APPROVED` readiness for that scope. The other four methods remain fail-closed `REQUIRED` with `0 / 0` executions. Evidence is [`accounting-browser-advanced-gates-latest.json`](evidence/accounting-browser-advanced-gates-latest.json). The development identity refuses Production startup, cannot coexist with OIDC and rejects non-local return URLs. Source-bound method, reviewed-journal and execution journeys are locally verified in PostgreSQL; additional browser acceptance remains required before enabling the other advanced profiles. This is local acceptance evidence only.
