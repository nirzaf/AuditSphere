...
**Pinned implementation checkpoint:** `c214cb35743284b7c336ae5d193d4026afb81da0` (source and tests); execution-ledger evidence is recorded separately in `docs/execution/current-slice.md` and `docs/execution/status.json`.
...
Local verification is checkpoint-specific. At source checkpoint `c214cb35743284b7c336ae5d193d4026afb81da0`, the full Release solution build passed with 0 warnings/errors, all **315/315 tests passed with 0 skipped** (Domain 259/259, API 6/6, E2E 50/50), CI-style discovery matched 315, and EF reported no pending model changes. The E2E project took 6m29s and the domain project 8m08s in this run. The existing local database remains at its 91-migration development baseline; a prior loopback restore drill passed at that baseline, not at the latest source schema. QAR remains the default only for blank client accounting/reporting inputs; source/import and FX-policy currencies remain explicit. Deterministic financial-package exports include versioned XLSX, DOCX and PDF profiles with exact-byte persistence, authorized downloads and a runtime-enforced trusted workbook formula allowlist. Approved advanced-method schedules persist canonical source/input digests, validate method-specific inputs at approval, and feed guarded execution records with balanced current/comparative statements, method output manifests/digests, current component/group-revision checks, idempotent retry and separate reviewer approval that revalidates component inputs. A fixed candidate fixture produces balanced QAR current and comparative advanced-consolidation statements, and IFRS method-owner scope is recorded in `STE-METH-APP-001-addendum`; all five method-specific golden fixtures and built-in-browser journeys are locally verified, while required external acceptance remains outstanding before an advanced profile is treated as fully enabled. Local evidence does not establish live Microsoft-provider behavior, professional approval, signing acceptance, production recovery, or independent human review; the detailed capability inventory remains in `docs/execution/current-slice.md` and `docs/execution/status.json`.
...
| G16 | Multi-currency | Approved rate sets/policies, pinned foreign-operation translation, maker/checker results and per-line FX lineage exist; component translation results are calculation-versioned; source-snapshot-bound entity remeasurement is persisted with exact approved rate observations, deterministic input hash, independent revalidation and review-only UI. Direct and streamed GL imports now retain each line's original currency, original amount and functional posting in their normalized digest and stored source records. [R11, R15] | Remeasurement amounts/classifications and outstanding-item status are still operator-entered against evidence; imported GL lines are not presumed open at period end. Professional methodology approval, real-currency golden scenarios and any required live integration evidence remain separate. Nonexchangeable currencies and unsupported hyperinflation remain blocked. | P4 |
...
**Local calculation and workflow boundary:** `CurrencyRemeasurementCalculator` treats the supplied prior carrying amount as already expressed in functional currency. Monetary FX adjustment is the closing-rate functional-currency value less that carrying amount; historical-cost non-monetary inputs use the historical rate and do not produce an FX remeasurement gain/loss. `CurrencyRemeasurementService` persists a schedule whose item rows pin an exact engagement-owned `DocumentSnapshot`, source hash, approved rate row, approved policy, normalized inputs and computed results. A different scoped reviewer revalidates those sources and totals before approval. The workbench lists only currently scoped period/engagement contexts, saves an in-progress browser draft locally, and displays the submitted item-level lineage. Approval does not post a journal. Amounts and classifications are transcribed by the preparer rather than inferred from GL, so the source must be checked before approval; professional methodology approval and any release/records gates remain independent.
