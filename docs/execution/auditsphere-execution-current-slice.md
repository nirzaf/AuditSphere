...
This file records observed repository state only. The authoritative build contract is [`docs/auditsphere-requirements-system-specification-current.md`](../auditsphere-requirements-system-specification-current.md) v5.0. The public repository intentionally excludes tenant identifiers, user principals, credentials, tokens, secret values, and private-provider URLs.

## AI-navigability refactor — capability partials, code map and documentation authority

Code commit `e3ea9c678efe414f67dcf9a4e78483062a41c15d`, pushed to `master` and remote-confirmed at 2026-09-25.

- **Capability-focused partial files (no behavior change):** `ConsolidationService`, `ClientAccountingService`, `AccountingAnalysisService`, `AuditFieldworkService` and `FinancialStatementService` are split into capability partial files under `Application/Accounting/{Consolidation,ClientAccounting,Analysis,FinancialStatements}/` and `Application/Audit/Fieldwork/`, with unchanged public static APIs and namespaces. `AuditSphereDbContext` stays one context, split into `AuditSphereDbContext.<Module>.cs` partial configuration files; `OnModelCreating` call order and model metadata are unchanged and `dotnet ef migrations has-pending-model-changes` reports no drift. `ClientAccountingTests` splits into `tests/.../ClientAccountingTests/ClientAccountingTests.<Capability>.cs` partials with unchanged test semantics.
- **Code map and authority:** `docs/architecture/auditsphere-architecture-code-map.md` maps each business capability to its Domain/Application/persistence/UI/tests files and critical invariants; `docs/architecture/auditsphere-architecture-current-architecture.md` records the implemented architecture and the `CURRENT`/`APPROVED`/`PROPOSED`/`HISTORICAL_SOURCE`/`SUPERSEDED` document-authority vocabulary. `AGENTS.md` now points to the code map and adds the capability-file and Web-composes-Application rules; README's volatile test/migration counts are replaced by a pointer to `docs/execution/status.json` as the single authority.
- **Preserved-source status banners:** both `docs/AuditSphere_R2R_Task_Breakdown/source/ORIGINAL_*.md` documents now start with a visible `HISTORICAL_SOURCE` banner naming `AGENTS.md` and `docs/architecture/auditsphere-architecture-current-architecture.md` as the implementation authority. The task-pack validator requires the banner and excludes it when hashing, so the pinned requirement-text hashes (`tracking/pack_manifest.json`) still match the original bytes exactly; pack validation reports 0 errors and its self-tests pass 21/21.
- **Business-semantic test files:** empty `Class1.cs` placeholders are deleted; `UnitTest1.cs` becomes `TrialBalanceCalculatorTests.cs` (class name unchanged); `R2RPassTwoTests`/`R2RPassThreeTests`/`R2RRemainingModulesTests` become `AccountingRecordsApprovalTests`/`SourceAcceptanceAndComparativesTests`/`ProfilesChartsAndConsolidationTests`. Stale file-path citations in `coverage/02`, `coverage/04`, `auditsphere-accounting-module-requirements-current.md` and this ledger now point at the post-split files; the pinned-SHA historical links in `auditsphere-audit-user-stories-prototype-gap-closure-proposed.md` are intentionally left on their old revision.
- **Architecture guards:** `tests/.../ArchitectureGuardTests.cs` enforces the project reference direction (Domain references nothing inward-layered, Application references Domain only, Infrastructure references Application+Domain, never a host) as three xUnit assertions without an architecture framework.
- **Observed verification (2026-09-25):** Release build of `AuditSphereOps.slnx` passed with 0 warnings/errors; full Release suite passed 402/402 with 0 skips (Domain 336/336 including the three architecture guards, Api 6/6, E2E Playwright 60/60) against local PostgreSQL 18.6; `dotnet ef migrations has-pending-model-changes` reports no pending model changes; task-pack `validate` reports 0 errors and `test_task_status.py` passes 21/21. Hosted run `36166771849` targets the exact SHA and was in_progress at record time; run `36150741963` (`9d6975b`) completed success and is the last completed build gate. No schema migration, tenant operation or production effect in this slice.

## Re-review repair — fail-closed FX translation, stable replay identity and evidence reconciliation (RR-01…RR-09)

Code commit `6621cbf315c5e9899fc916069e34b5df8bf10327`, pushed to `master` and remote-confirmed at 2026-09-25.

- **RR-01 (stable reserve-line identity):** `ConsolidationService.BuildCalculationAsync` no longer mints `Guid.CreateVersion7()` for the cumulative-translation-reserve balance on each recalculation. The identity is derived from the immutable translation snapshot (`TranslationIdentities.CumulativeTranslationReserveLineId`), so an identical retry returns the same run ID and run hash, independent approval succeeds without input modification, and `GetLatestReportAsync` keeps reporting `CURRENT_APPROVED`. Observed by `ClientAccountingTests:ForeignOperation_Gold07_ReplayApprovalAndReadbackAreStable`.
- **RR-02 (no missing-rate fallback):** `CurrencyTranslationService.TranslateComponentAsync` no longer falls back to one aggregate rate when a required rate purpose is absent; a missing/zero/negative rate purpose returns a typed `gate.blocked` failure and no translation row is persisted. Same functional and presentation currency is identity, not a missing rate: `TranslateComponentAsync` and `ApproveTranslationAsync` record and revalidate rate exactly 1 with no rate observation (a same-currency observation is unrepresentable because `AddRateAsync` refuses `from == to`). Observed by `ForeignOperation_MissingRequiredAverageDoesNotFallback` and `ForeignOperation_MissingRequiredHistoricalDoesNotFallback` (each asserts failure, the named missing purpose and an empty `TranslationResults` set), `RateSet_RejectsZeroNegativeAndSameCurrencyObservations` (zero, negative and same-currency observations refused at the rate-set boundary), `ForeignOperation_SameCurrencyComponentTranslatesAsIdentity` (identity rate 1 through translation and independent approval), `ForeignOperation_AssetOnlyProfileNeedsOnlyClosingRate` (a valid asset-only profile needs only the closing rate and produces zero reserve), `LineTranslationTests:MissingRatesFailClosed` (missing/zero/negative/unspecified rates throw at the bridge) and `:SameCurrencyNeedsNoRate` (identity at the bridge).
- **RR-03 (method selects per-line rates, not the reserve amount):** `AddBalance` and `ApproveTranslationAsync` select per-line rate purposes from `TranslationResult.CalculationVersion == COMPONENT_TRANSLATION_V2` instead of `translation.TranslationReserve != 0`; a missing purpose rate at group build fails instead of substituting the header rate. Observed by `ForeignOperation_ZeroReserveStillUsesPerLinePurposes` (Cash 400, Payables −480, Capital −280, Revenue −144, Expense 504, no reserve line, signed sum 0 with a zero reserve), now through the full implemented chain: translation approval → group calculation → separate-partner `ApproveRunAsync` → `GetLatestReportAsync` returning `CURRENT_APPROVED` with Capital −280 / Revenue −144 / Expense 504 intact. The export/archive leg is a declared M37/M38 boundary (`modules/auditsphere-r2r-module-26-consolidation-contract.md` "Group-report release boundary"): M37 controlled release revalidating freshness at freeze/issue and M38 archiving the exact released bytes are not implemented in this blueprint, and `RecordsArchiveService` archives `DocumentSnapshots` only, so group-report export/archive is excluded from this chain's evidence rather than claimed.
- **RR-04 (exact per-line rate identity and explicit classification):** `ConsolidationCalculator.Compute` resolves each manifest row's balance by `(ComponentId, SourceLineId)` and records that line's own consumed rate purpose and rate. `LineTranslationCalculator.InferSection` no longer defaults unknown codes to ASSETS: `TryInferSection` returns null and `TranslationInputResolution` resolves a line only from a recognized declared section, an approved mapping allocation or documented code rules, otherwise raising an explicit unmapped issue. Observed by `ForeignOperation_ManifestUsesExactLineRateIdentity` and `ForeignOperation_UnknownTaxonomyRequiresApprovedMapping` (unmapped blocks; one approved mapping resolves it; two conflicting mappings block as ambiguous).
- **RR-08 (T075 source meanings and audit-acceptance gate):** T075's Outcome and sequential work now cite `AWP-17-01`…`AWP-17-06` with their preserved source procedures, cite `AWP-17-07`/`AWP-17-08` for classification and financial-statement reflection, and declare an explicit "Audit acceptance and handover gate" (reviewed-through date, procedure dispositions, classification decisions, reflection evidence, traceability reconciliation, operator handover record) for `AS-AUD-028-AC10`. T073/T074 citations were corrected the same way, and 21 further task cards now carry the preserved source wording for every cited AWP ID. The pack validator rejects source-procedure meaning drift and undeclared handover inputs; `tools/test_task_status.py` covers both (`test_source_ids_preserve_original_procedure_meaning`, `test_audit_handover_requires_declared_acceptance_inputs`) and passes 21/21 with `validate` reporting 0 errors.
- **RR-05 (recorded architecture variation):** `docs/AuditSphere_R2R_Task_Breakdown/reference/auditsphere-r2r-reference-baseline-architecture-and-adrs.md` §2.3.1 records that commands and queries are static services returning `CommandResult`/`CommandResult<T>` rather than MediatR handlers, and that no bUnit component-test project exists (neither package is pinned); the transaction-ownership registry rules apply unchanged and reintroducing either remains a separate reviewed package change.
- **RR-06/RR-07 (evidence reconciliation):** `coverage/auditsphere-r2r-tracker-acceptance-criteria.md` VP-044-AC02/AC03/AC04 now name the exact observing tests and record that the blanket `b75b476` claim for VP-044-AC02 was over-broad while a fail-open fallback still defaulted amounts. `coverage/auditsphere-r2r-tracker-golden-fixtures.md` GOLD-R2R-07 policy approval is `NOT_APPROVED` because the earlier `APPROVED` cell named test methods instead of the firm methodology owner; fixture approval stays T003's human decision and the numerical expectations remain `PASS` against the named tests.
- **RR-09 (external providers stay blocked):** `GraphPbcProviderSink` still returns `live-provider-not-approved` for Upload/Verify/Probe. No Purview, eSignature, payment or AI provider was introduced; external gates P1–P5 and P7–P10 remain `BLOCKED_EXTERNAL` and need an approved environment plus human sign-off (T052–T054).
- **Observed verification (2026-09-25):** Release build of `AuditSphereOps.slnx` passed with 0 warnings/errors; full Release suite passed 396/396 with 0 skips (Domain 330/330 including the seven `ForeignOperation_*` acceptance cases, Api 6/6, E2E Playwright 60/60) against local PostgreSQL 18.6; `dotnet ef migrations has-pending-model-changes` reports no pending model changes; task-pack `validate` reports 0 errors and `test_task_status.py` passes 21/21. Hosted GitHub Actions remains the mandatory build gate only (pinned SDK, locked restore, Release build, EF pending-model check) and executes no tests, so hosted green is not test acceptance; README records this boundary. No schema migration, tenant operation or production effect in this slice.
- **Follow-up lifecycle and rate-boundary evidence (2026-09-25):** Completion-verifier gaps closed in code commit `43b01f97a56c56b4e9bfadc4fc5afce8fb529505`, pushed to `master` and remote-confirmed. Same functional/presentation currency is identity rate exactly 1 through `TranslateComponentAsync` and `ApproveTranslationAsync` with no rate observation consumed (`AddRateAsync` refuses `from == to`, so none exists). `ForeignOperation_ZeroReserveStillUsesPerLinePurposes` now asserts Capital −280 / Revenue −144 / Expense 504 through a separate partner's `ApproveRunAsync` and `GetLatestReportAsync` (`CURRENT_APPROVED`, no reserve line, signed sum 0). Zero/negative/same-currency rate observations are refused at the rate-set boundary (`RateSet_RejectsZeroNegativeAndSameCurrencyObservations`), a same-currency component translates and is independently approved as identity (`ForeignOperation_SameCurrencyComponentTranslatesAsIdentity`), a valid asset-only profile needs only the closing rate (`ForeignOperation_AssetOnlyProfileNeedsOnlyClosingRate`), and `LineTranslationTests:MissingRatesFailClosed` now names the negative-rate case alongside zero. The group-report export/archive leg is recorded as a declared M37/M38 boundary (`modules/auditsphere-r2r-module-26-consolidation-contract.md` "Group-report release boundary"; VP-044-AC03 evidence cell) instead of being left uncovered. Observed: Release build 0 warnings/errors; full Release suite passed 399/399 with 0 skips (Domain 333/333, Api 6/6, E2E Playwright 60/60) against local PostgreSQL 18.6; `dotnet ef migrations has-pending-model-changes` reports no pending model changes; task-pack `validate` reports 0 errors and `test_task_status.py` passes 21/21. Hosted run 36150267317 targets the exact SHA and was in_progress at record time; prior repair runs are observed as 36145744612 (`062cca6`) success and 36145506279 (`6621cbf`) cancelled as superseded. No schema migration, tenant operation or production effect in this follow-up.

## IAS 21 Translation Rate Bridge, Consolidation Integration & Task Breakdown Reconciliation (F01–F08)

- **F01 (IAS 21 Rate Bridge):** Updated `LineTranslationCalculator` in `CurrencyOperationCalculators.cs` to calculate translation reserve strictly from the rate bridge: equity difference between closing rate and historical rate (-40) plus profit & loss difference between closing rate and weighted average rate (-8), totaling -48. Added support for opening translation reserve roll-forward. Signed translated total balances to identically 0.00, satisfying `GOLD-R2R-07`. Verified by unit tests in `LineTranslationTests.cs` (8/8 pass).
- **F02 & F03 (Multi-Rate Translation & Consolidation Integration):** Connected per-line translation through `CurrencyTranslationService` and `ConsolidationService`. Foreign component lines are translated at individual purpose rates (Closing for assets/liabilities, Historical for equity, Average for income/expense) using `ResolvePurpose()`. When a non-zero translation reserve exists, a `CTA_RESERVE` line (`CumulativeTranslationReserveSection`) is added to consolidation balances, balancing group totals to identically 0.00. Verified with end-to-end integration test `ForeignOperation_GoldR2R07_TranslatesPerLineWithExplainedReserveAndConsolidates` in `ClientAccountingTests/ClientAccountingTests.Currency.cs` (1/1 pass).
- **F04 & F05 (Acceptance & Golden Fixture Ledgers):** Marked `GOLD-R2R-07` as `APPROVED` / `PASS` in `auditsphere-r2r-tracker-golden-fixtures.md` with references to `LineTranslationTests.cs` and `ClientAccountingTests/ClientAccountingTests.Currency.cs`. Recorded verified evidence for `VP-044-AC02..AC04` in `auditsphere-r2r-tracker-acceptance-criteria.md`.
- **F06 (T054 & T075 Task Scope Reconciliation):** Scoped task card T054 explicitly as Record-to-Report only production acceptance and handover. Moved `AS-AUD-028-AC10` from T054 to T075 in task cards, `auditsphere-audit-tracker-workflow-traceability.md`, and refreshed `auditsphere-r2r-index-task-breakdown.md`. Validated with `test_task_status.py` (19/19 tests pass) and `task_status.py validate` (75 tasks, 0 errors).
- **F07 & F08 (Scope & Verification Documentation Alignment):** Aligned `README.md` with `AGENTS.md` specifying that Purview and eSignature provider integrations are excluded from product scope (preserving exact uploaded signed-document evidence, SHA-256 identities, human decisions, and release manifests without claiming provider integration). Updated test case counts to 334.
...
- Added [E2E automation strategy and embedded CI examples](../testing/auditsphere-testing-e2e-automation-strategy.md) and the [complete existing-test catalog](../testing/auditsphere-testing-test-case-catalog.md).
