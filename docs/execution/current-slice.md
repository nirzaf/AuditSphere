...
- **Code map and authority:** `docs/architecture/CODE_MAP.md` maps each business capability to its Domain/Application/persistence/UI/tests files and critical invariants; `docs/architecture/CURRENT_ARCHITECTURE.md` records the implemented architecture and the `CURRENT`/`APPROVED`/`PROPOSED`/`HISTORICAL_SOURCE`/`SUPERSEDED` document-authority vocabulary. `AGENTS.md` now points to the code map and adds the capability-file and Web-composes-Application rules; README's volatile test/migration counts are replaced by a pointer to `docs/execution/status.json` as the single authority.
- **Preserved-source status banners:** both `docs/AuditSphere_R2R_Task_Breakdown/source/ORIGINAL_*.md` documents now start with a visible `HISTORICAL_SOURCE` banner naming `AGENTS.md` and `docs/architecture/CURRENT_ARCHITECTURE.md` as the implementation authority. The task-pack validator requires the banner and excludes it when hashing, so the pinned requirement-text hashes (`tracking/pack_manifest.json`) still match the original bytes exactly; pack validation reports 0 errors and its self-tests pass 21/21.
- **Business-semantic test files:** empty `Class1.cs` placeholders are deleted; `UnitTest1.cs` becomes `TrialBalanceCalculatorTests.cs` (class name unchanged); `R2RPassTwoTests`/`R2RPassThreeTests`/`R2RRemainingModulesTests` become `AccountingRecordsApprovalTests`/`SourceAcceptanceAndComparativesTests`/`ProfilesChartsAndConsolidationTests`. Stale file-path citations in `coverage/02`, `coverage/04`, `AuditSphere_Accounting_module.md` and this ledger now point at the post-split files; the pinned-SHA historical links in `AuditSphere_Prototype_Gap_Closure_User_Stories.md` are intentionally left on their old revision.
...
- **F04 & F05 (Acceptance & Golden Fixture Ledgers):** Marked `GOLD-R2R-07` as `APPROVED` / `PASS` in `04_Golden_Fixture_Tracking.md` with references to `LineTranslationTests.cs` and `ClientAccountingTests/ClientAccountingTests.Currency.cs`. Recorded verified evidence for `VP-044-AC02..AC04` in `02_Acceptance_Criteria_Tracking.md`.
- **F06 (T054 & T075 Task Scope Reconciliation):** Scoped task card T054 explicitly as Record-to-Report only production acceptance and handover. Moved `AS-AUD-028-AC10` from T054 to T075 in task cards, `AUDIT_WORKFLOW_TRACEABILITY.md`, and refreshed `00_INDEX.md`. Validated with `test_task_status.py` (19/19 tests pass) and `task_status.py validate` (75 tasks, 0 errors).
