...
- Fifteen applied migrations on `auditsphere`, ending at `20260918161647_MappingAndFinancialStatementWorkflow`; `dotnet ef database update` reported no pending migrations. The new migration refuses ambiguous legacy financial-package rows, adds scoped mapping/adjusted-TB/package tables and append-only artifact triggers, and records the package review gate for missing supplementary information.
- Build: zero warnings/errors. Full suite: 93/93 passed, zero skipped. The suite now also covers incomplete mapping rejection, reviewer approval/input-generation fencing, deterministic/idempotent financial-package creation, explicit supplementary-information review gating, database immutability of package lines, and refusal of ambiguous legacy financial-package rows.
...
- Restore rehearsal: `scripts/db/restore-drill.sh` dumped and restored `auditsphere` into a generated temporary loopback database, verified fifteen migrations with latest `20260918161647_MappingAndFinancialStatementWorkflow`, then cleaned its generated database and temporary files.
...
The web readiness check now verifies both PostgreSQL reachability and an empty `GetPendingMigrationsAsync()` result. `.github/workflows/ci.yml` provisions PostgreSQL 18, restores/builds/tests the solution, applies migrations to the disposable CI database and probes readiness. `Directory.Build.props` now enables committed NuGet lock files; the workflow uses `dotnet restore --locked-mode`, fixing the setup-dotnet cache failure caused by missing `packages.lock.json`. `scripts/db/restore-drill.sh` is constrained to loopback PostgreSQL 18 tooling, restores to a generated temporary database, checks the fifteen-migration history, and removes only generated artifacts. Local build, full suite 92/92, restore rehearsal, and `Healthy`/HTTP 200 readiness passed. The superseded hosted run could not start because the GitHub account Actions billing/spending limit failed; production/cross-store recovery remains unobserved.

## Mapping and financial-statement package follow-up — local verified

`FinancialStatementService` now creates and approves versioned source-to-taxonomy mappings, reconstructs an adjusted trial-balance snapshot from a finalized adjustment plan, and emits deterministic scoped package lines/totals plus persisted validation results. Package status remains `REVIEW_REQUIRED` because supplementary information, disclosures and cash-flow workings are not supplied. The migration is local-only; no provider, tenant record, signing or production assertion is made. Six `packages.lock.json` files are committed so `actions/setup-dotnet@v4` caching has the required inputs.

## Not complete or production-enabled

The initial Blazor shell is present, but the full staff/client route catalog, upload transport, scoped forms for the remaining commands, and authorized operator recovery UI remain unfinished. Mapping/FS rendering, disclosures/cash-flow inputs, real provider adapters, and full cross-store recovery also remain unfinished. Accounting, outbox, CRM, time/budget, billing, bounded firm-ledger, document-snapshot, approval, release, and the new mapping/package scoped FKs/checks and append-only controls now exist; remaining module chains still need integrity work. TB rows reject UPDATE/DELETE, but this slice does not complete the future promoted-dataset insert freeze/import lifecycle. Validation retains a table-level row-writer lock and loads one TB into memory; no workload claim is made. Readiness and local restore proof do not establish hosted CI or production recovery. Checklist #13 remains partial and #14 remains locally verified but hosted-CI blocked; external checklist #15–16 remain blocked.
