...
| Remote | `origin/master` points to `c451d08` after the asset-schedule methodology checkpoint push |
...
| Migrations | 49 applied; latest `20260920222544_AddAssetScheduleMethodology` |
| Model drift | `dotnet ef migrations has-pending-model-changes` — no changes |
| Restore drill | `scripts/db/restore-drill.sh` — passed; 49 migrations reconciled |
...
- Typed GL import, bounded paged reads, account-by-account trial-balance-to-GL completeness bridges, reconciliation workbenches, generation-bound ECL/inventory/specialist/analytical/journal-risk workbenches, and explicit asset-schedule depreciation-method/useful-life/closing-balance controls.
...
The restore evidence is written to [`docs/evidence/restore-drill-latest.json`](../evidence/restore-drill-latest.json). The latest rehearsal restored 49 migrations through `20260920222544_AddAssetScheduleMethodology`; it is loopback-only and explicitly reports that external checkpoint custody and production RPO/RTO were not run.
