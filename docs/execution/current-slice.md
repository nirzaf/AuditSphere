...
- **RR-02 (no missing-rate fallback):** `CurrencyTranslationService.TranslateComponentAsync` no longer falls back to one aggregate rate when a required rate purpose is absent; a missing/zero/negative rate purpose returns a typed `gate.blocked` failure and no translation row is persisted. Observed by `ForeignOperation_MissingRequiredAverageDoesNotFallback` and `ForeignOperation_MissingRequiredHistoricalDoesNotFallback` (each asserts failure, the named missing purpose and an empty `TranslationResults` set).
