...
| Item | Observed value |
|---|---|
| Source implementation checkpoint | `master@ccb01ea` |
| Remote | `origin/master` includes source checkpoint `ccb01ea` |
| SDK | .NET 10; repository solution targets `net10.0` |
| Database | PostgreSQL 18.6 on loopback port 5433 for development only |
| Build | `dotnet build AuditSphereOps.slnx --no-restore --configuration Release` — passed, 0 warnings/errors |
| Tests | 212/212 passed, 0 skipped against PostgreSQL 18.6 |
...
- Portfolio projections apply the same active firm-wide, client or engagement grant boundary to counters, holds, durable operations, release candidates and financial-package lists; actors without an explicit scope see no firm data and receive actionable guidance.
