...
- Twenty-four applied migrations on `auditsphere`, ending at `20260919111301_ReleaseEvidenceAndCheckpoints`; `dotnet ef database update` reported no pending migrations. Adds `release_checkpoints`, `protection_attestations`, `signature_lineages`, composite scope foreign keys (`ON DELETE RESTRICT`), replaces `releases.external_checkpoint` (bool) with `releases.checkpoint_id` (UUID NOT NULL FK to `release_checkpoints`), and installs database immutability triggers.
- Build: zero warnings/errors across all 6 projects. Full suite: 157/157 passed, zero skipped locally on PostgreSQL 18.6 development cluster. New coverage: `ReleaseEvidenceTests` (NT-19 pre-sign ≠ signed hash lineage preservation, unsigned release blocked under `RequireSignatureLineage=true`, expired protection attestation blocked under `RequireProtectionAttestation=true`, `ReleaseSafetyOptions` startup validation refusal of live activation without external effects, and `LocalAppendOnlyCheckpointStore` content-addressed integrity and conflict detection) plus updated `ReleaseTests` verifying stored read-back checkpoint enforcement and release immutability.
...
- Restore rehearsal: `scripts/db/restore-drill.sh` dumped and restored `auditsphere` into a generated temporary loopback database, compared restored migration evidence with the source, verified twenty-four migrations with latest `20260919111301_ReleaseEvidenceAndCheckpoints`, then cleaned its generated database and temporary files.
