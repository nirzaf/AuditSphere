...
- The consolidation workbench links each advanced scope to a scope-bound browser workflow for source-manifest/input drafting, guarded schedule submission, separate schedule/execution approval and execution status. Draft inputs restore locally, required fields are marked, and no browser action bypasses server-side maker/checker or readiness gates. The development harness still shows the seeded profiles as `REQUIRED` with `0 / 0` executions; the workflow smoke test covers the no-approved-input case only.
