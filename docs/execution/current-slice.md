...
## Current verified baseline — master @ `1322bc3`

**Current head:** `1322bc36113a43e3f6a1bdb36f5e04cd78949bea` (raw Entra claim mapping fix and tenant evidence refresh, 2026-09-20)
...
## Local OIDC runtime verification (observed 2026-09-20)

- The running development host at `http://localhost:5099` completed the Entra callback for the already signed-in `qts@easyguide.onmicrosoft.com` account and rendered live PostgreSQL Portfolio counts for firm `11111111-1111-1111-1111-111111111111`.
- After the local cookie was signed out, `/app` rendered `Sign-in required` and explicitly stated that no local demo actor is available. Selecting the qts Entra account completed the localhost callback again and restored Portfolio. This is local protected-shell evidence only; it does not close the full real-tenant fixture matrix.
- After setting the OIDC handler to preserve raw Entra `oid`/`tid` claims (`MapInboundClaims=false`), a fresh qts sign-in in a separate Chrome tab completed the localhost callback and rendered Portfolio with live PostgreSQL counts for the same firm. The staff identity now maps through the immutable tenant/object lookup; no demo actor is used.

## Tenant evidence (EasyGuide, observed 2026-09-18 through 2026-09-20)
...
- On 2026-09-19, Entra showed the single-tenant `AuditSphereOps Development` app (`29be1ee5-e90c-4ecc-b4f9-5bcce14774cc`) with Microsoft Graph `Sites.Selected` application permission granted for EasyGuide. The later credential readback below supersedes the earlier no-credential observation.
- On 2026-09-20, Entra showed the same single-tenant app with web redirect URI `http://localhost:5099/signin-oidc`, zero certificates, and one current client secret labeled `AuditSphereOps Development non-production runtime` expiring 2027-03-19. Only masked secret metadata was read; the secret value was not revealed. The qts localhost callback is verified separately; Client A/B, disabled/revoked, changed-UPN, and wrong-tenant fixture acceptance remains open.
- On 2026-09-20, Infisical project `AuditSphereOps` in the `Development` environment displayed four masked runtime secret references: `Identity__CallbackPath`, `Identity__ClientId`, `Identity__ClientSecret`, and `Identity__TenantId`. No secret value was revealed or changed. This confirms named non-production references exist in the selected project, but the UI alone does not establish approved rotation/custody evidence or prove runtime consumption.
- On 2026-09-20, the same Infisical project showed empty `Staging` and `Production` environments; no additional fixture credentials or production runtime references were present there. The `Development` environment remains the only environment with the four masked OIDC references.
- On 2026-09-20, Infisical Approval Workflows showed zero open and zero closed change requests and no approval policies. Project Audit Logs were not available on the Free plan, which leaves secret-change custody/rotation history unobservable in the selected project.
- The local host was restarted with the Infisical-backed OIDC handoff without logging the secret value. A read-only `GET /auth/sign-in?returnUrl=/app` returned HTTP `302` to the EasyGuide tenant authorization endpoint, and the existing qts session remained authenticated in the browser. This verifies local OIDC runtime consumption; approved rotation/custody evidence remains open.
...
- On 2026-09-20, the Purview Records Management file plan showed `AuditSphere Dev Test Record` as Active with one-day retention, record locking, disposition review, and reference ID `AS-DEV-REC-001`. Label policies showed both `AuditSphere Dev Test Record Publication` and `AuditSphere Dev Test Label Policy` as Enabled; the publication details described the selected AuditSphere Development site scope and the `AuditSphere Dev Test Record` label.
- On 2026-09-20, SharePoint Documents received the synthetic developer record `AuditSphere-Dev-Test-Record.txt` (3 bytes) in the selected AuditSphere Development site. SharePoint `Compliance details` read back `This item is not subject to a retention policy`, `Hold Status: Not on hold`, and `In Place Records Management Status: Not an in place record`. This is direct negative protection-state evidence: publication is enabled, but label propagation/application is not yet observed.
- On 2026-09-20 at `09:23:29Z`, a fresh SharePoint `Compliance details` reread returned the same negative state for `AuditSphere-Dev-Test-Record.txt`: no retention policy, not on hold, and not an in-place record. The local authenticated Portfolio shell remained available in the same browser session.
- On 2026-09-20, Entra temporarily changed `AuditSphere P0 Client Y Fixture` from `auditp0-client-y@easyguide.onmicrosoft.com` to `auditp0-client-y-renamed@easyguide.onmicrosoft.com` and then restored the original UPN. The immutable object ID remained `8e64950c-e8f2-48f9-aeae-be15f0e82939` and Account status remained Enabled after restoration. This is UPN-stability fixture evidence only; no client sign-in acceptance was performed.
- On 2026-09-20, the EasyGuide Entra Users inventory reported 16 users. Searching `AuditSphere P0` returned only enabled Member fixtures: Client X (`bb5e8021-8c86-46eb-80e3-1b13f1caedbf`, `auditp0-client-x@easyguide.onmicrosoft.com`), Client Y (`8e64950c-e8f2-48f9-aeae-be15f0e82939`, `auditp0-client-y@easyguide.onmicrosoft.com`), and Staff (`99eaa47e-d18d-4e7f-90d7-804e20a46fd3`, `auditp0-staff@easyguide.onmicrosoft.com`). Exact searches for Client A, Client B, `disabled`, and `revoked` returned zero users. No wrong-tenant/guest fixture was observed. This confirms the required Client A/B, disabled/revoked, and wrong-tenant identities are not provisioned in this tenant; no account was created or altered during the inventory.
