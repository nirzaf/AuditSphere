...
- Five applied migrations on `auditsphere`: `InitialCreate`, `TrialBalanceValidation`, `AccountingIntegrity`, `20260917104422_DurableOutbox`, and `20260918041428_AuthorizationIntegrity`. Verified 46 public tables, the `ux_users_tenant_subject` index, grant/engagement/hold FKs and checks, and unchanged `numeric(19,6)` TB amounts after this local migration.
- Build: zero warnings/errors. Full suite: 51/51 passed, zero skipped (40 prior cases plus 11 authorization-decision cases). The original 12 regressions remain, including five arithmetic tests; the AJ arithmetic test still does **not** establish persisted source reflection.
