...
| Item | Observed value |
|---|---|
| Source implementation checkpoint | `master@c9504f2` |
| Remote | `origin/master` includes source checkpoint `c9504f2` |
| SDK | .NET 10; repository solution targets `net10.0` |
| Database | PostgreSQL 18.6 on loopback port 5433 for development only |
| Build | `dotnet build src/AuditSphereOps.Web/AuditSphereOps.Web.csproj --no-restore --configuration Release` at `c9504f2` — passed, 0 warnings/errors |
| Tests | 215/215 passed, 0 skipped against PostgreSQL 18.6 at `c9504f2` |
...
- The accounting workspace exposes scoped COA/mapping, adjustment-journal and difference queues with exact-record links, removes the invalid unscoped journal route, and shows a period workflow dashboard with required/complete/stale/blocked counts, role-based next-owner guidance, persisted in-scope PBC owner/due-date values when available, an existing-PBC handoff link, and an explicit `Not recorded` due-date state when no persisted task/PBC due date exists. Release build, full PostgreSQL suite and local HTTP smoke for the new routes pass at `c9504f2`.
- Group consolidation exposes perimeter, component-pack, FX, intercompany and elimination tabs with persisted scope counts/statuses; same-currency and unsupported-method boundaries remain explicit. Release build, full PostgreSQL suite and local HTTP smoke for `/app/consolidation` pass at `c9504f2`.
- Package-review selection is draft-retained through the shared storage fallback layer and offers a non-mutating preview that separates eligible stages from partner-role blockers; it never records a partial approval. Release build and the 215-test PostgreSQL suite pass at `c9504f2`.
