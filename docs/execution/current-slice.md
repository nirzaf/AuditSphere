...
- Eleven applied migrations on `auditsphere`: `InitialCreate`, `TrialBalanceValidation`, `AccountingIntegrity`, `20260917104422_DurableOutbox`, `20260918041428_AuthorizationIntegrity`, `20260918083524_AdjustmentSourceBridge`, `20260918093347_PracticeCrmWorkflow`, `20260918095500_CrmSafetyInvariants`, `20260918111815_PracticeTimeBudgetWorkflow`, `20260918115026_BillingArtifactWorkflow`, and `20260918121512_FirmLedgerWorkflow`. `dotnet ef database update` reported no pending migrations.
- Build: zero warnings/errors. Full suite: 81/81 passed, zero skipped. The suite includes CRM, time/budget, billing, and ledger tests for finance-role separation, finance-profile gating, invoice/credit/receipt balance enforcement, concurrent source-allocation uniqueness, balanced/idempotent firm posting, deferred multi-row balance enforcement, posted-history immutability, period-close/posting race fencing, and refusal of ambiguous legacy ledger rows.
...
## Checklist #9 — bounded firm ledger (implemented, commit `ea40814`)
...
The web host still exposes status/health endpoints only; mapping/FS production, document snapshots, approvals, release gates, lifecycle screens, real provider adapters, and full cross-store recovery remain unfinished. Accounting, outbox, CRM, time/budget, billing, and bounded firm-ledger scoped FKs/checks and append-only controls now exist; remaining module chains still need integrity work. TB rows reject UPDATE/DELETE, but this slice does not complete the future promoted-dataset insert freeze/import lifecycle. Validation retains a table-level row-writer lock and loads one TB into memory; no workload claim is made. Health checks still prove connectivity only. Checklist #10–14 remain not started; external checklist #15–16 remain blocked.
