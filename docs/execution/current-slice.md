...
- Thirteen applied migrations on `auditsphere`: `InitialCreate`, `TrialBalanceValidation`, `AccountingIntegrity`, `20260917104422_DurableOutbox`, `20260918041428_AuthorizationIntegrity`, `20260918083524_AdjustmentSourceBridge`, `20260918093347_PracticeCrmWorkflow`, `20260918095500_CrmSafetyInvariants`, `20260918111815_PracticeTimeBudgetWorkflow`, `20260918115026_BillingArtifactWorkflow`, `20260918121512_FirmLedgerWorkflow`, `20260918125731_DocumentSnapshotIntegrity`, and `20260918131715_ApprovalApplicabilityWorkflow`. `dotnet ef database update` reported no pending migrations.
- Build: zero warnings/errors. Full suite: 88/88 passed, zero skipped. The suite includes CRM, time/budget, billing, ledger, document-snapshot, and approval tests for finance-role separation, finance-profile gating, invoice/credit/receipt balance enforcement, concurrent source-allocation uniqueness, balanced/idempotent firm posting, deferred multi-row balance enforcement, posted-history immutability, period-close/posting race fencing, exact-byte SHA-256 capture, duplicate source-version rejection, snapshot/approval append-only protection, stale revision/generation rejection, and refusal of ambiguous legacy rows.
...
## Checklist #11 — revision/generation-bound approvals (implemented, commit `94a8da3`)
...
The web host still exposes status/health endpoints only; mapping/FS production, release gates, lifecycle screens, real provider adapters, and full cross-store recovery remain unfinished. Accounting, outbox, CRM, time/budget, billing, bounded firm-ledger, document-snapshot, and approval scoped FKs/checks and append-only controls now exist; remaining module chains still need integrity work. TB rows reject UPDATE/DELETE, but this slice does not complete the future promoted-dataset insert freeze/import lifecycle. Validation retains a table-level row-writer lock and loads one TB into memory; no workload claim is made. Health checks still prove connectivity only. Checklist #12–14 remain not started; external checklist #15–16 remain blocked.
