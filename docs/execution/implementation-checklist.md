...
**Current source checkpoint:** `master@b7305c1`; PostgreSQL 18.6; 80 migrations; 218/218 tests passed; 0 skipped. Trial-balance imports, client-scoped GL dimensions and chart/taxonomy hierarchy (including posting-parent rejection and approved taxonomy statement-section alignment), QAR-defaulted accounting setup, GL service-date lineage, opening/movement completeness, capability-profile scope validation, and context-bound financial packages and adjustment journals are bound to the validated reporting period, optional book and basis/currency context. Financial-package validation now records separate cross-cast, accounting-equation, equity/profit, comparative-consistency and note-to-face outcomes, with key lineage identifiers in the deterministic artifact. Exact rendered package bytes are persisted with framework/template versions and SHA-256 lineage, and package-review decisions cannot be recorded without the matching artifact. Analytical reviews now preserve QAR-defaulted reporting currency, deterministic replay snapshots/hashes, movement flags and reviewer conclusions; secured aggregate summaries enforce client/engagement or explicit group scope and omit component identifiers; journal-risk evidence preserves sample-selection, management-explanation and corroboration lineage, while bounded analysis returns criteria-versioned manual/year-end/high-value/reversal/missing-origin review indicators without making automatic fraud findings. Linked differences now persist exact-journal account/statement effects, profit/equity totals, disclosure buckets, approved mapping lineage and a `journal-impact.v2` hash, while correction records retain materiality/qualitative metadata and governed state transitions with stale-impact rejection. Bank reconciliation binds independently approved ledger and statement schedules with typed timing/proposed-correction lineage and approval/completion gates. Queued durable operations can be cancelled with an explicit administrator disposition before worker claim, and cancelled work is excluded from publication. ECL differences reconcile to an explicit booked amount and ECL/inventory evidence can retain same-engagement proposed-journal lineage; receivable/payable reconciliations now retain explicit ageing basis, bucket rule, bucket, credit treatment and paired settlement links; audit-difference summaries now preserve gross and signed/net corrected/unadjusted totals by currency. GL-backed bank-ledger schedules now derive their control total from a sealed, scoped, legal-entity-bound, hash-matching GL import batch with complete account coverage; account-code matching is case-insensitive and caller-supplied mismatches fail closed. The latest local slice also enforces service-permissibility decisions, versioned FX effective ranges, approved translation-rate rules, separate persisted FX/rounding adjustments and deterministic advanced-consolidation calculation cores with acquisition fair-value adjustments; approved advanced-method golden fixtures remain pending.
