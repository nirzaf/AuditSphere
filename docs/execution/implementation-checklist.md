...
Dependency-ordered per spec Appendix G and the "first proving slice". One item at a time: build + PostgreSQL-backed tests + commit before moving on. Statuses: ✅ done · 🟡 partial · ⬜ not started · 🚫 blocked (external).
...
| 3 | Durable outbox: claim/lease/attempt fence/idempotency-key uniqueness | ND-03, ADR-06, §29 | Two workers claim distinct ops; retry does not duplicate side effects | ⬜ |
...
- **#3 — next**: durable outbox claim/lease/attempt-fence/idempotency (ND-03, §29).