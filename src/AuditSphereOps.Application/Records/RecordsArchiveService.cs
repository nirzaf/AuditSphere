...
public static class RecordsArchiveService
{
    ...
    IAuditSphereDbContext db, Archive archive, CancellationToken ct)
  {
    ...
    var eclAssessments = typedAccounting is null ? [] : await typedAccounting.EclAssessments.AsNoTracking()
      .Where(x => x.FirmId == archive.FirmId && x.ClientId == archive.ClientId && x.EngagementId == archive.EngagementId).OrderBy(x => x.Id)
      .Select(x => new { x.Id, x.ReconciliationId, x.ReconciliationSourceHash, x.InputGeneration, x.Version, x.AsOfDate, x.Method, x.MethodologyVersion, x.EligibleExposure, x.ProbabilityOfDefault, x.LossGivenDefault, x.ManagementOverlay, x.CalculatedExpectedLoss, x.ManagementExpectedLoss, x.BookedAmount, x.Difference, x.ProposedJournalId, x.AssumptionsHash, x.Status, x.CreatedByUserId, x.ReviewedByUserId, x.CreatedAt, x.ReviewedAt }).ToListAsync(ct);
    var inventoryAssessments = typedAccounting is null ? [] : await typedAccounting.InventoryValuationAssessments.AsNoTracking()
      .Where(x => x.FirmId == archive.FirmId && x.ClientId == archive.ClientId && x.EngagementId == archive.EngagementId).OrderBy(x => x.Id)
      .Select(x => new { x.Id, x.ReconciliationId, x.ReconciliationSourceHash, x.InputGeneration, x.Version, x.AsOfDate, x.Quantity, x.UnitCost, x.NrvPerUnit, x.ObsolescenceReserve, x.BookAmount, x.CalculatedAmount, x.Difference, x.ProposedJournalId, x.MethodologyVersion, x.AssumptionsHash, x.Status, x.CreatedByUserId, x.ReviewedByUserId, x.CreatedAt, x.ReviewedAt }).ToListAsync(ct);
    ...
    var analyticalReviews = typedAccounting is null ? [] : await typedAccounting.AnalyticalReviews.AsNoTracking()
      .Where(x => x.FirmId == archive.FirmId && x.ClientId == archive.ClientId && x.EngagementId == archive.EngagementId).OrderBy(x => x.Id)
      .Select(x => new { x.Id, x.PeriodId, x.InputGeneration, x.ComparisonPeriodId, x.Area, x.Measure, x.CurrentAmount, x.PriorAmount, x.BudgetAmount, x.Ratio, x.Currency, x.DenominatorBasis, x.FormulaVersion, x.MovementFlags, x.SeasonalityExplanation, x.InputSnapshotJson, x.InputHash, x.Explanation, x.ReviewConclusion, x.Status, x.CreatedByUserId, x.ReviewedByUserId, x.CreatedAt, x.ReviewedAt }).ToListAsync(ct);
    var journalRiskFlags = typedAccounting is null ? [] : await typedAccounting.JournalRiskFlags.AsNoTracking()
      .Where(x => x.FirmId == archive.FirmId && x.ClientId == archive.ClientId && x.EngagementId == archive.EngagementId).OrderBy(x => x.Id)
      .Select(x => new { x.Id, x.ImportBatchId, x.TransactionId, x.RuleCode, x.Reason, x.Score, x.SelectedForTesting, x.ManagementExplanation, x.CorroborationReference, x.Status, x.Disposition, x.EvidenceReference, x.CreatedByUserId, x.ReviewedByUserId, x.CreatedAt, x.ReviewedAt }).ToListAsync(ct);
